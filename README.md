<p align="center">
  <br />
  <img width="100" src="./open ai.png" alt="OpenAI logo">
  <br />
  <br />
</p>

<h1 align="center">Awesome ChatGPT Plugins</h1>

<p align="center">
An awesome &amp; curated list of best plugins for ChatGPT.
</p>

## What Are ChatGPT Plugins?

[![Watch Video](https://res.cloudinary.com/marcomontalbano/image/upload/v1679667398/video_to_markdown/images/video--a267d1cba9d59411634ce2088744acf9-c05b58ac6eb4c4700831b2b3070cd403.jpg)](https://cdn.openai.com/chat-plugins/retrieval-gh-repo-readme/Retrieval-Final.mp4 "Watch Video")

ChatGPT plugins are tools designed specifically for language models with safety as a core principle, and help ChatGPT access up-to-date information, run computations, or use third-party services.

OpenAI plugins connect ChatGPT to third-party applications. These plugins enable ChatGPT to interact with APIs defined by developers, enhancing ChatGPT's capabilities and allowing it to perform a wide range of actions.

## Official Resources

- [Getting started With Creating A ChatGPT Plugin](https://platform.openai.com/docs/plugins/getting-started)
- [Example plugins](https://platform.openai.com/docs/plugins/examples)
- [Plugin policies](https://openai.com/policies/usage-policies#plugin-policies)

## Official Plugins

- [ChatGPT Retrieval Plugin](https://github.com/openai/chatgpt-retrieval-plugin) - The ChatGPT Retrieval Plugin repository provides a flexible solution for semantic search and retrieval of personal or organizational documents using natural language queries. 
- [Browsing](https://openai.com/blog/chatgpt-plugins#browsing) - An experimental model that knows when and how to browse the internet.
- [Code interpreter](https://openai.com/blog/chatgpt-plugins#code-interpreter) - An experimental ChatGPT model that can use Python, handle uploads and downloads.

## Plugins From ChatGPT Plugin Store (1037 Plugins)

 **Plugin Name**             | **Plugin Description**                                                                                                       
-----------------------------|------------------------------------------------------------------------------------------------------------------------------
 **A Bestever Ad Maker**     | Unlock stunning image ads with just a link. Our Bestever AI scripts, polishes your visuals, and generates magic!             
 **A Code Library Help**     | Write code and answer questions using Python libraries (e.g Openai, Twilio, etc). Can answer version specific questions      
 **A&amp;A Cards by For.io** | Create beautiful social media cards and cheatsheets with summary lists or data tables, from any text as input.               
 **A&amp;B Chat With PDF**   | Analyze PDFs by uploading documents or providing URLs. Extract info, query content and unlock insights swiftly!              
 **A&amp;B Summarize!**      | Create summaries of documents (PDF, Word), web pages, books, reviews, YouTube, Reddit &amp; Wikipedia.                       
 **A&amp;B Web Search!**     | Browse the web using Google search. Discover the latest information online by entering keywords or phrases.                  
 **A/B Analytics**           | A/B test and analyze data from Google Analytics, Facebook Ads and more. Powered by Avian.io.                                 
 **A/B JUDGE**               | Judge whether the A/B test results are superior or not.                                                                      
 **Aalii FileChat**          | File management, in-depth analysis, and quick information retrieval.                                                         
 **AANote**                  | An AI-Powered chat summarization and article generation tool.                                                                
 **Aardvark AI**             | Search for Products on Google Shopping in Real-Time, No Ads, Only Trusted Stores.                                            
 **Aardvark News**           | Get the latest news stories from around the world, generate tweets and digests from &gt;80,000 trusted global news           
 **Aaron Browser**           | I’ll scrape data from multiple website URLs. Built for Internet crawling, content aggregation, and monitoring.               
 **Aaron Build Resume**      | Create impressive professional resume/CV from scratch or update an existing one. Export as PDF and .docx.                    
 **Aaron Code Review**       | I’ll review GitHub code, summarize it &amp; suggest improvements. Submit Git commit details or URL/link for insights.        
 **Aaron Confluence AI**     | Chat with your Confluence wiki. Share your Confluence pages (and other links!) as context, and get personalised…             
 **Aaron Copywriter**        | I’ll rewrite articles per your instructions. Send me web pages, PDFs, Word docs, Wikipedia, reviews, anything!               
 **Aaron Cover Letter**      | Create impressive cover letters using your resume and job details. Ideal for job seekers and career changers.                
 **Aaron Docs™ Chat**        | Get an oracle that can answer questions based on your Google Docs™, Slides™ and Sheets™.                                     
 **Aaron Fill PDF Forms**    | Fill legal forms &amp; complex PDF documents easily! Upload a file, provide data sources and I’ll handle the rest.           
 **Aaron Notion™ Chat**      | Chat with your Notion™ wiki. Share your Notion pages (and other links!) as context, and get personalised answers.            
 **ABC Music Notation**      | Converts ABC music notation to WAV, MIDI, and PostScript files.                                                              
 **ABC Website Maker**       | Create web apps, websites, custom code, and more, easy as ABC! Turn prompts/chats into apps in seconds!                      
 **Able**                    | Use store addons without install! Simply type ‘Able PlugName’ for 1-click testing of chat extension and instant.             
 **Able Network Monitor**    | Monitor websites, email servers, check dns resolution, ping services and check for quantum safe encryption.                  
 **Able Style Fashion**      | Able Style is a fashion assistant who will help you answer the question, ‘What shall I wear today?’                          
 **Abridged Due Diligence**  | Discover the details! Search through recent SEC filings, with links to deeper analysis.                                      
 **Access Google Sheet**     | Ask your Google Sheets questions &amp; chat with excel. Enter Google Drive link to start!                                    
 **Access Link**             | Access any links on the web and get the information you need.                                                                
 **Access PDF &amp; Docs**   | Ask your PDF questions, summarize info &amp; chat with PDF! Enter Google Drive link to start!                                
 **Access PDF Export**       | Generate beautiful PDFs in seconds, from cover letters, resumes, charts, graphs, contracts &amp; lists.                      
 **Accurate PDF**            | Interactive conversations with PDFs of any length, with accurate quotes and page citations. Powered by Avian.io              
 **Acquire.com**             | Everything you need to buy and sell startups.                                                                                
 **ad4mat**                  | Generate affiliate tracking links to monetize outgoing traffic – based on 50,000+ advertisers.                               
 **Adzviser**                | It retrieves users’ real-time reporting data from Google Ads, Facebook Ads, Bing Ads, GA4 and more.                          
 **AeroDex**                 | Aviation data from aviationweather.gov for entertainment. Not for pre-flight decision making.                                
 **Affilitizer**             | Find all affiliate programs and advertisers including the network they are available in.                                     
 **AFinChat**                | Get information and analyze A-share stocks, but search for information only from China.                                      
 **AgentSQL**                | Interact with your Snowflake databases or your csv files.                                                                    
 **Agones**                  | Agones provides soccer (football) results for matches played all over the world in the past 15 years.                        
 **AI Agents**               | Unleashing the power of multiple AIs: One goal, limitless productivity.                                                      
 **AI Browse and Sear**      | Combine Google search results with AI, research information from the internet and browse webpages.                           
 **AI Data Analyst**         | Dive into your data like never before. No code, no complex queries; just plain English.                                      
 **AI Diagrams**             | Create and edit Whimsical Flowcharts and Mind Maps.                                                                          
 **Ai Drive**                | Chat with with all your pdfs files and more. Keep everything organized in your personal Ai Drive.                            
 **AI Extensions**           | Craft your extensions with ease! Step-by-step guidance at your fingertips.                                                   
 **AI Gift Finder**          | Use the power of AI to find the perfect gift.                                                                                
 **AI News Roundup**         | Get today’s AI news headlines as a clickable link roundup!                                                                   
 **Ai PDF**                  | Super-fast, interactive chats with PDFs of any size, complete with page references for fact checking.                        
 **AI Quest**                | An interactive text adventure. Your choices shape the AI-crafted narrative. Each playthrough is a unique story and game.     
 **AI Search Engine**        | Search and browse the internet/web using Google Search (SERP) with AI Search Engine. Summarize &amp; chat with PDF and code. 
 **AI SEO MY SITE**          | Get site score of a website.                                                                                                 
 **AI Theme Finder**         | I’ll find and compare top WordPress themes for your next blog or website. Just provide your preferences.                     
 **Ai Tool Hunt**            | Explore the ideal AI solutions for all use cases, drawn from the most comprehensive global database of AI tools.             
 **AI Video Chat**           | Ask questions to a youtube video.                                                                                            
 **AI Video Summarize**      | Ask questions, analyzing, and parsing through YouTube videos by simply providing a YouTube video URL.                        
 **AI2sql**                  | Converts a natural language text into an SQL query.                                                                          
 **AI4ERP**                  | Plugin for asking questions related to SAP, including transaction codes, business processes, Fiori Tiles/Apps, etc.          
 **AILC BioChem**            | AI Learning Companion for Biology and Chemistry. Does 2D and 3D molecule diagrams with highlights and anatomy                
 **AILC History**            | Draw maps of kingdoms, empires and battles. Search for images to illustrate what you are learning. Show timelines.           
 **AIMaster**                | The prompt is too difficult, now you only need to provide the desired prompt keywords, and you can use the prompt.           
 **AIPLUX TM**               | Analyse your business and recommend NICE classifications and items to file for trademarks.                                   
 **AISEO Article Writer**    | Create Serp-based, NLP friendly articles within minutes.                                                                     
 **AISEO Repurposer**        | Convert any article or YouTube video into blogs, tweets, emails, scripts, press releases, landing pages &amp; more.          
 **AIstrologer**             | Search for the horoscope for each zodiac sign for a specific date.                                                           
 **AITickerChat**            | Retrieve USA stock insights from SEC filings as well as Earnings Call Transcripts.                                           
 **AladinBooks**             | Search for books on Aladin.                                                                                                  
 **Alarm Advisor**           | Get expert advice on Telenot alarm systems and personalized support for your home and business security.                     
 **AlbaDOC**                 | Your trusted source for Canadian drug information.                                                                           
 **Alchemy**                 | Request real-time blockchain data for chains like Ethereum, Polygon, Arbitrum and Optimism through natural language.         
 **Algorand Explorer**       | Algorand blockchain explorer and search with enriched augmented data. Built by GoPlausible!                                  
 **Algorithma**              | Shape your virtual life with in this immersive life simulator game to begin Type /start to begin.                            
 **Alpha News**              | Alpha News provides access to &gt;80k news sources, letting you search for topics, publications, and trending                
 **AlphaNotes**              | Transform YouTube video courses into your personal study guide, making learning efficient and enjoyable.                     
 **AmazingTalker**           | Elevate your language learning at any level with personalized 1-on-1 online lessons from tutors across the world.            
 **Amazon Finds**            | Find any Amazon product you need.                                                                                            
 **Amazon Gift Chooser**     | Curated gifts for every occasion and special person.                                                                         
 **Ambition**                | Search millions of jobs near you.                                                                                            
 **AMBOSS**                  | Find reliable medical information provided by AMBOSS, a trusted medical source for thousands of physicians and…              
 **AnaBot**                  | AnaBot is a conversational guide for well-being industry insights and trends.                                                
 **AnalyticsAI**             | Connect to your Google Analytics™ and get data with AI using simple prompts.                                                 
 **Analyze Act**             | An API that provides access to data stored in Analyze Act, collected from Google Analytics.                                  
 **Andorra**                 | All of Andorra with the power of AI.                                                                                         
 **Angi Pro Finder**         | Find top-rated certified pros in your area for all your home projects &amp; services.                                        
 **AnyBarChart**             | Create captivating bar charts with ease. Customize labels, colors, and titles for a tailored visual experience!              
 **AnyPDF**                  | Easily Extract PDFs from URLs &amp; convert text to PDFs.                                                                    
 **AnySheet**                | Create CSV files and Excel sheets effortlessly and securely.                                                                 
 **AnySlides**               | Easily Generate Stunning PowerPoint Presentations from Your Data or Any URL.                                                 
 **APEX Map**                | Checking the current APEX Legends Ranked Map.                                                                                
 **API Bot**                 | This is a conversational bot that lets you ask questions about a variety of common APIs.                                     
 **Apify**                   | Scrape Google Search results, crawl websites and query web pages found, and browse individual web pages.                     
 **AppCreator**              | A cloud-native platform designed for rapid and automated web application development and testing.                            
 **Appy Pie App Builder**    | AI-powered Text-to-App Generator turns your app idea into Android and iOS apps- just provide text input.                     
 **Argil AI**                | Generate images without learning how to prompt.                                                                              
 **ArtCollection**           | Search through millions of art pieces from The Metropolitan Museum of Art.                                                   
 **ArticleCreator**          | An AI-based platform designed for rapidly and automatically writing all kinds of articles.                                   
 **arXiv Xplorer**           | Discover, read, reference, and search through all arXiv papers.                                                              
 **ASH**                     | ASH by lagogal.com. Currently it provides currency converter.                                                                
 **Ask About Austin**        | A tool that allows the user to get information about Austin Poor.                                                            
 **Ask Link Bot**            | Conversational bot to ask a question about a link and receive an answer based on that.                                       
 **Ask MFM**                 | Ask the hosts of the My First Million questions about business, tech, entrepreneurship, and life.                            
 **Ask VG**                  | Search through VG’s content archive.                                                                                         
 **AskChloe**                | Discover your style and get personalized recommendations on clothing, accessories, gifts, and more.                          
 **AskForQrcode**            | Generate QR code for any any requirement, Text, Url , V card and more.                                                       
 **AskTheCode**              | Provide a GitHub repository URL and ask about any aspect of the code.                                                        
 **AskYourCode**             | Ask your code without having to copy-paste it. Works with most popular programming languages.                                
 **AskYourDatabase**         | Chat with your databases without writing any SQL code.                                                                       
 **AskYourKG**               | Eliminate hallucination. Organize your knowledge. Chat with it. Visualise it.                                                
 **AskYourPDF**              | Unlock the power of your PDFs!, dive into your documents, find answers, and bring information to your fingertips.            
 **AskYourPDF Pro**          | Unlock the power of your documents with the pro version of AskYourPDF! Bring information to your fingertips today.           
 **ASO**                     | App Store Optimization by Phiture: keyword popularity, estimate impressions &amp; uncover trends.                            
 **Aspose Words Cloud.**     | Allows you to get content of documents stored in Aspose Words Cloud.                                                         
 **Asset Ovi**               | Search and preview millions of 3D assets for games, AIGC, AR/VR and others.                                                  
 **Asteroid Alert**          | Interact with NASA to explore earth observation data that monitors near-earth objects like asteroids.                        
 **Astro Insights**          | A companion simplifying complex astrophysics tasks.                                                                          
 **Astrodaily**              | Search and discover NASA images.                                                                                             
 **Astrology**               | Discover your cosmic DNA. Dive into your exact birth (natal) chart for growth, purpose, and healing.                         
 **ATSResumeCopilot**        | Tailor your resume for a job description &amp; get discovered By ATS. Say ‘Review my Resume’. Powered by Maigrate.           
 **Aurora**                  | A conversational bridge to your Philips Hue Lights. Shape your ambiance with a simple chat.                                  
 **Aus Petrol Prices**       | Ask for the average daily petrol price for any state or capital city region in Australia!                                    
 **Aus Surf Report**         | Get today’s surf report for any break throughout Australia!                                                                  
 **Austrian Bank Rates**     | Compare the rates and terms of checking accounts and savings accounts from Austrian banks.                                   
 **Auth Network Monit**      | Elevate network monitoring with real-time, quantum-ready, HTTP, ICMP, DNS, SMTP checks, this tool alerts you 24/7.           
 **Auto Prompt**             | Save time by queuing your prompts to run one after the other.                                                                
 **AutoInfra1**              | Talk to your Servers. Works with AWS, GCP, Azure, and anywhere you can ssh!                                                  
 **Autolab Causality**       | Calls Autolab to compute bounds on causal queries.                                                                           
 **Autoleaf**                | Autoleaf reads raw LaTeX from your paper library to assist you in writing scientific articles.                               
 **Avalara**                 | Calculate sales tax or lookup tax rates for any address in the U.S.                                                          
 **Avian**                   | Connect to your business data – Google Analytics, Google Ads, Facebook Ads and more.                                         
 **B12 AI Websites**         | Create a professional, engaging, and user-friendly website for your business in seconds using AI.                            
 **B12 Email marketing**     | Send high-quality emails in minutes that engage contacts and sell your services within minutes.                              
 **B12 Scheduling**          | Bring online scheduling to your business and enable your customers to book appointments in minutes.                          
 **Back4App**                | Use natural language to create, deploy, and scale apps, backend, databases, cloud functions, web pages, and APIs.            
 **Bardeen**                 | Create and run automations on the most popular web services.                                                                 
 **BART Real-Time**          | Getting real-time BART information for a specified origination station and direction.                                        
 **BeatandRaise**            | Edgar SEC data on demand. Get income statements, balance sheets, management comments.                                        
 **BeeBee.AI**               | BeeBee.AI delivers key insights on financial reports and earnings calls for everyday investors.                              
 **BGG Assistant**           | Discover board games with BoardGameGeek Assistant! Find games, see trending games, monitor your plays and collection…        
 **BibGuru**                 | Supercharge your research: generate accurate citations in APA, MLA, and Harvard.                                             
 **Bible**                   | Explore Bible’s content with detailed interpretations, comparisons, and feature to find related verses.                      
 **Bibliography Crossref**   | Search publications and generate bibtex bibliographies using Crossref and Zotero.                                            
 **Bibsonomy**               | Helps you to research scientific publications, manage your Bibsonomy library and analyze its content.                        
 **BILD News**               | Stay up-to-date with Germany’s leading source for the latest news on politics, sports, entertainment, and more.              
 **BioRxiv Researcher**      | An application leveraging Google Custom Search for the most comprehensive BioRxiv research…                                  
 **Bitcoin Sentiment**       | Track the current price of Bitcoin and the market sentiment based on the last 20 news media mentions!                        
 **BizToc**                  | Search BizToc for business &amp; finance news.                                                                               
 **BlendED Worksheet**       | Create educational resources (question papers, bubble sheets, worksheets, etc.) instantly from your teaching material.       
 **BlockAtlas**              | Search the US Census! Find data sets, ask questions, and visualize.                                                          
 **blue News**               | Stay updated with the latest news from blue News.                                                                            
 **Bohita**                  | Create apparel with any image you can describe! Get it delivered right to your door.                                         
 **Bonrix Dynamic QR**       | Generate UPI QR Sticker to pay with easy-to-scan QR codes for UPI transactions by passing UPI ID and…                        
 **Book Tickets**            | Explore and secure tickets for a variety of events around the world.                                                         
 **Bookworm**                | Personalized book recommendations, scanning countless titles to find your perfect read via AI.                               
 **Boolio Invest**           | Analyze stock investments from any country, with Boolio’s state-of-the-art engine.                                           
 **BOXIL SaaS**              | Help Japanese companies find the best system to enhance their productivity.                                                  
 **Bramework**               | Boost SEO with in-depth analysis, including keyword insights on volume, ranking, and SERP.                                   
 **Brandfetch**              | Retrieve company and brand data including logos, colors, fonts, and other brand information.                                 
 **Broadway**                | Search for shows that are currently playing on Broadway in New York City.                                                    
 **Browser**                 | Pilot your research with real-time Google search results. Scrape &amp; extract content from webpages by providing a…         
 **BrowserOp**               | Browse dozens of webpages in one query. Fetch information more efficiently.                                                  
 **BrowserPilot**            | Deliver real-time search results, fetch and analyze info in multiple URLs.                                                   
 **Bubble Goods**            | Marketplace of 1000+ tasty &amp; healthy foods. Discover new Vegan, Keto, Gluten-Free products &amp; more.                   
 **BuildBetter**             | Chat with the knowledge of all your calls in BuildBetter (Zoom, GMeet, Webex). Start for free @ BuildBetter.ai               
 **BuyWisely**               | Compare Prices &amp; Discover the Latest Offers from thousands of online shops in Australia.                                 
 **BUZZ.NEW**                | BUZZ.NEW: A smart business creator for business management. Create, manage inventory, and handle orders efficiently.         
 **ByByAI**                  | Search for the best Amazon products ordered by rating.                                                                       
 **C-Level Resume**          | I’ll create an ATS-optimized C-Level executive resume, designed to land your senior-level job.                               
 **Calculator**              | A calculator app that executes a given formula and returns a result. This app can execute basic and advanced                 
 **Calculator Tools**        | Create any app, calculator, tool, tracker, or game instantly with AI.                                                        
 **California Law**          | Get up to date access to California law.                                                                                     
 **Calorie Coach**           | Track meals &amp; gain insights for a healthier lifestyle from 1m+ restaurants &amp; grocery stores.                         
 **Canar.AI Job Agent**      | Find job &amp; career insights from the top 10 platforms to match your skills with titles powered by Canar.AI!               
 **Canva**                   | Create anything in a snap, from presentations and logos to social media posts and more.                                      
 **CapCut**                  | Submit your video ideas, and let AI craft a script, find suitable footage, and merge everything into a polished video.       
 **Career Copilot**          | A personal AI Agent that finds jobs for software engineers. Partner with Commit.dev to find a better job.                    
 **CarGurus**                | Find Your Perfect Car on CarGurus – Shop by Budget, Deal Rating, Make, Model, &amp; More.                                    
 **CarYardBard**             | AI-Powered Car Sales Ad Generator for Australian Car Dealers.                                                                
 **Catch Them All**          | Catch Them All – Catch, Battle, and get information about your favourite Pokemon                                             
 **CCData.io**               | Access the latest crypto prices and news aggregated by CCData.io (formerly CryptoCompare.com).                               
 **CCS – Spext**             | Chat with the Conscious Creators Show podcast library on Spext.                                                              
 **Cello Square**            | Search for quote, booking shipment, tracking and more value with logistics services in Samsung SDS.                          
 **Chabad Centers**          | Find information about Chabad Centers worldwide and their events.                                                            
 **Chaca Market Product**    | Get detailed information about products on Chaca Market                                                                      
 **Chainstack**              | Enables natural language interaction with EVM blockchains using Chainstack’s infrastructure.                                 
 **ChampDex**                | Chat with your favorite League of Legends champions!                                                                         
 **Change**                  | Discover impactful nonprofits to support in your community and beyond.                                                       
 **Charge My EV**            | Locate EV charging stations by various providers, including Tesla supercharger locations.                                    
 **CharitySense**            | Get data on US-based non-profits including mission, key people, governance, ratings and financial data.                      
 **Charts &amp; Diagrams**   | Mermaid Diagrams, Schemes for Presentations, Analysis, research websites, pie charts.                                        
 **Charts by Kesem AI**      | Generate charts. The user can request a chart to be generated.                                                               
 **Chat TMDB**               | Movies, Actors and TV shows from TMDB, a database like IMDB.                                                                 
 **Chat Tool Finder**        | Find the tool in here that can help you. Search by name, category or description.                                            
 **Chat with Bible**         | Intuitively explore scriptures. Find relevant verses, understand parables, and gain insights.                                
 **Chat with calendars**     | Manage your Google Calendar. View events, schedule appointments, and stay organized.                                         
 **Chat With Excel**         | Transform your spreadsheet interactions and analysis by conversing as if you were a friend.                                  
 **Chat With Many URLs**     | Enter multiple URLs and chat with their text!                                                                                
 **Chat With Workspace**     | Chat with your Notion workspace.                                                                                             
 **Chat With Your Data**     | Conversational analysis over your databases and spreadsheets.                                                                
 **ChatAFI**                 | Search through select AFIs.                                                                                                  
 **ChatBlog**                | Use AI to write quality blog like humans. Work for your SEO rankings, not just efficiency!                                   
 **ChatFeedSync**            | Easily synchronize, manage, and translate your RSS feeds all in one place.                                                   
 **ChatOCR**                 | The best way to read text from any document. Extract text from scanned PDFs, photos, and even handwriting.                   
 **ChatRakuJourney**         | Search for Rakuten services in Japan. You can easily search for products, facilities in travel, and more.                    
 **ChatSSHPlug**             | Ability to SSH into your server and turn your natural language into server commands.                                         
 **ChatStackSearch**         | Use the Stack Exchange API to search sites such as Stack Overflow and return questions, answers, and other                   
 **ChatWeb3**                | Query and analyze blockchain and crypto data using natural language.                                                         
 **ChatWithCode**            | Analyze, query, and generate code from Private and Public Github repos.                                                      
 **ChatWithFriend**          | Easily create an invoice or estimate as a PDF file.                                                                          
 **ChatWithGit**             | Search code on GitHub repositories based on a query.                                                                         
 **ChatWithInstructions**    | Store and use multiple Custom Instructions.                                                                                  
 **ChatWithPDF**             | Chat with everything from entire PDF books to Google Drive documents just by providing a link.                               
 **ChatWithWebsite**         | Have a conversation with any website, powered by magicform.ai                                                                
 **Check Point Security**    | Check Point security reputation about IP, URL, File hashes – Get their current risk and classification.                      
 **Check Website Down**      | Insert your website URL to check the availability. Pulsetic tests the uptime, availability &amp; performance of a website.   
 **Checkers**                | This allows you to play a game of checkers.                                                                                  
 **Checkmarx CheckAI**       | Security Code Scanning for generated source code. Ensuring a secure development environment.                                 
 **CheckTheChain**           | Look for anything on the blockchain and get instant analysis.                                                                
 **Chess**                   | Unleash your inner chess master with this interactive chess experience! You can play against a novice or a grandmaster!      
 **Chi Data Buddy**          | Chicago data from the City of Chicago Data Portal.                                                                           
 **Chithra**                 | Chithra: An all-in-one image editor that uses DALL-E 2 for creating, generating, manipulating, and enhancing photos          
 **City of Alara**           | Dive into the game of Alara! Explore magic-filled districts and bond with captivating characters.                            
 **Clay**                    | Ask questions and search across your network and contacts — Linkedin, Facebook, email, and iMessage.                         
 **Clearbit**                | Access Clearbit Enrichment, Prospecting, Reveal APIs and website visitors data to access information about companies.        
 **Climesumer**              | Find sustainable alternatives to products.                                                                                   
 **Clinical Trial Radar**    | Clinical trial discovery tool for patients, caregivers, HCPs &amp; Biotech pros.                                             
 **CLINQ**                   | Manage your CLINQ Account. Retrieve infos about calls, create call reminders and more.                                       
 **Close CRM**               | Access your Close CRM data. Understand your leads, get deeper insights, and make them actionable.                            
 **Clothing Shopper**        | Get the absolute best clothing deals through AI recommendations.                                                             
 **Cloud Diagram Gen**       | Generate cloud architecture diagrams. Unofficial AWS plugin.                                                                 
 **Cloudflare Radar**        | Get real-time insights into Internet traffic patterns and threats as seen by Cloudflare.                                     
 **CMoney Portfolio**        | Analyze Taiwan stocks and get exclusive investing data from leading fintech CMoney in Taiwan.                                
 **Code Runner**             | Run and Save your code while creating visualizations (charts and graphs) supports upto 70 programming languages.             
 **Codecademy**              | Access our vast database of educational tech and coding resources to help you in your learning journey.                      
 **CodeCast Wandbox**        | Instantly compile your code online with Wandbox. Improve your coding learning experience.                                    
 **CoderPad**                | Run code in more than 30 different programming languages, and let us worry about setting up the environment!                 
 **Codeulator**              | Get read/write access to files in your Visual Studio and VS Code projects.                                                   
 **CoffeeHo**                | Your Personal Coffee Expert.                                                                                                 
 **CoinCap**                 | Get cryptocurrency information from CoinCap.                                                                                 
 **CoinMarketCap**           | Get the latest crypto data from CoinMarketCap.                                                                               
 **Cold Email Generator**    | Generate cold emails to increase your engagement and grow your business.                                                     
 **Color Palette**           | Craft vibrant palettes from your ideas.                                                                                      
 **CommonEvents**            | Search nearby events in Korea.                                                                                               
 **Companies In The UK**     | Provides financial information on UK Companies.                                                                              
 **Company Enrichment**      | Get company information by providing the domain name. Useful for sales teams.                                                
 **Company Transcripts**     | Search and analyze the latest company transcripts for any stock (powered by Koyfin).                                         
 **Comparison**              | An advanced e-commerce tool, providing robust capabilities for efficient product search and accurate price comparison.       
 **Conceptmap**              | A companion to structure ideas to conceptmaps.                                                                               
 **Consensus Search**        | Get answers &amp; insights directly from 200M+ scientific papers. To use, just ask a research question.                      
 **Content Rewriter**        | Enter any URL and have the text content rewritten for you!                                                                   
 **Content.app**             | Effortlessly buffer &amp; post Social, SEO, Ad content. Supports WordPress, Twitter, YouTube, Shopify, Medium + more!        
 **Contentify Marketing**    | Analyze and evaluate every aspect of your website’s marketing. Ask ‘How can you improve your website to maximize…            
 **Convert Assistant**       | Provides guidance regarding Convert.com’s AB testing tool.                                                                   
 **Converter App**           | Convert currencies or files like images and videos from web links and Google Drive to other formats, or work with PDFs.      
 **CoPilot**                 | Searches every dealer, analyzes &amp; ranks every car for you so you can buy with confidence.                                
 **Copywriter**              | Enter any URL and get sales copywriting suggestions for the page!                                                            
 **Coq Checker**             | Check Coq code.                                                                                                              
 **Coupert**                 | Search for the internet’s best coupons from thousands of online stores.                                                      
 **Coupons by Tenere**       | TenereTeam.com – Get the Best Deals &amp; Coupons on Millions of Products At Over 50,000 Online Stores.                      
 **Course Hero**             | Get course-specific study materials from Course Hero’s library.                                                              
 **Coursera**                | Find recommendation for courses, specializations, and degrees on Coursera.                                                   
 **Crafty Clues**            | Guess the words that the AI craftily clues for you. Add restrictions to make the game more interesting!                      
 **CranePumps Manuals**      | Returns the catalog and manual for a pump based on model number.                                                             
 **Create a QR code**        | Create a QR code for any text or url.                                                                                        
 **CreatiCode Scratch**      | Display Scratch programs as images and write 2D/3D programs using CreatiCode Scratch extensions.                             
 **Creative Mind**           | Unleash your imagination with Creative Mind, transforming text prompts into stunning visual art.                             
 **Creatuity Stores**        | We integrate stores so you can search for products in all of them at the same time.                                          
 **CreditYelp**              | Access various essential financial calculators for a detailed repayment schedule and payoff term.                            
 **Cribbage Scorer**         | Tool for scoring your cards in the game of cribbage.                                                                         
 **Crossword Genius**        | Get hints, solutions and explanations to cryptic crossword clues.&nbsp;                                                      
 **Crypto Jobs List**        | Find jobs and talent profiles in Crypto, Blockchain and Web3 industries.                                                     
 **Crypto Market News**      | It’s your go-to solution for real-time cryptocurrency price updates, market insights, and the latest news.                   
 **Crypto Price Checker**    | A Crypto Prices app that takes a pair of crypto or fiat tickers and returns the current price of the pair.                   
 **Crypto Pulse**            | From News to Profit: Decode Crypto’s Market Impact with Ease. Instantly, analyse latest crypto news.                         
 **Crypto Staking Bot**      | Сrypto research assistant by Everstake. Real-time updates on APY, price, total stake data and more for PoS…                  
 **CryptoMation**            | Crypto transactions, addresses, balances, conversions for any blockchain (Ethereum, Binance, Near, Tron, etc.)               
 **CSV Creator**             | Create a CSV file given text.                                                                                                
 **CSV Export**              | Create and export custom CSV layouts in a flash.                                                                             
 **CSV Export Plus**         | Create and export custom CSV layouts in a flash, with auto-save to Google Sheets.                                            
 **CT Criteria Parser**      | Analyze eligibility criteria in ClinicalTrials.gov. Example input: nctid NCT05859269                                         
 **Currency Converter**      | Convert currencies using the CoinGecko API.                                                                                  
 **Currency Converter**      | Convert currencies based on real-time rates!                                                                                 
 **Currency Converter**      | Convert currencies based on real-time rates.                                                                                 
 **Currency Today**          | Converts currency values based on the latest exchange rates.                                                                 
 **Custom Knowledge**        | Provide your docs, webpages, and other links, and get personalised answers.                                                  
 **CVEs**                    | Look up Common Vulnerabilities and Exposures (CVEs).                                                                         
 **Dafny Checker**           | Check that Dafny code passes the verifier, without executing the code.                                                       
 **Daily Learning**          | Unlock your full potential with a personalized education journey featuring custom-tailored lesson plans!                     
 **DailyBot**                | Smart insights from your team daily stand-ups, check-ins, surveys, or knowledge base forms.                                  
 **DAIZY**                   | ETF Research, Stocks, Cryptos, Performance, Risk, Sustainability, Investment, Portfolio Analysis.                            
 **Dart**                    | Project management on autopilot.                                                                                             
 **Data Interpreter**        | Analyze your data using a Python interpreter in a secure, sandboxed execution environment.                                   
 **Datasheet.Chat**          | Gateway to interactive, and intelligent chat with any electrical components datasheet.                                       
 **DayApp**                  | Get connected with any company representative without waiting on hold or pressing buttons in IVR.                            
 **Deal Dazzle**             | Search and compare prices from numerous online stores. Find top coupons from various shops. (US only)                        
 **Dear Diary**              | Effortlessly capture and reflect on your daily memories.                                                                     
 **Decision Diagrams**       | Create and edit diagrams directly in chat.                                                                                   
 **Decision Journal**        | Become a better decision maker by keeping track of your decisions and reviewing how they turn out.                           
 **Decision Matrix**         | Get decision and return back a matrix of decision types.                                                                     
 **DecisionConsultant**      | Using the most powerful AI to simulate top human experts for professional decision-making support.                           
 **Deep Memory**             | Create flashcards and review them on Deep Memory, a spaced repetition app.                                                   
 **Defillama**               | Retrieve data on DeFi protocols and blockchains.                                                                             
 **Definitive Facts**        | Ask questions using 100+ relational datasets – sports, finance, and more at https://definitive.io/datasets.                  
 **Denison Yachting**        | Search yachts for sale, charters, new construction, crew placement, and more with Denison Yachting.                          
 **DEV Community**           | Recommending posts and members from DEV Community.                                                                           
 **Developer Doc Search**    | Access thousands of open source packages and their GitHub documentation effortlessly.                                        
 **Devhunt**                 | Find your next favorite tool.                                                                                                
 **Diagrams**                | Create and display diagrams from kroki.io or using networkx and matplotlib.                                                  
 **Diagrams: Show Me**       | Schemes, Diagrams, Architecture Visualisations, Flow-Charts, Mind Maps. Export and Edit for free!                            
 **Diary**                   | Kickstart your diary journey. Keeping a diary is designed to be effortless and beneficial.                                   
 **Diffbot**                 | Access the world’s knowledge and real-time Web data with Diffbot’s Knowledge Graph.                                          
 **Digital Pets**            | Take care of your digital pet, go on adventures, learn tricks, and interact with others digital pet owners!                  
 **Directual**               | Directual is a full-stack no-code platform that is great for launching web and web3 apps of any complexity and scale.        
 **Disteur**                 | Quickstart for creating a product draft on the Disteur platform.                                                             
 **DM Tool Kit**             | App for rolling dice using the d20 or Fate/Fudge systems.                                                                    
 **DobaWorldFactory**        | World Factory: Searching for the best manufacturers worldwide.                                                               
 **Doc Editor**              | Create beautiful docs instantly, with powerful editing features. Export to PDF, DOCX, Google Drive, OneDrive, and…           
 **Doc Maker**               | Generate beautiful PDFs in seconds. Resumes, cover letters, proposals and more. Supports DOCX, XLSX, CSV and..               
 **Doc Maker A+**            | Generate A+ quality docs in seconds. Resumes, cover letters, and more. Supports PDFs, DOCX, XLSX, CSV and…                   
 **Doctor’s Life Career**    | Searching Japanese medical doctors’ jobs based on your preferences.                                                          
 **Document AI**             | The only Document AI that shows you where it got your answer from.                                                           
 **Domain Finder AI**        | Verify if a domain is available by searching across multiple domain registrars.                                              
 **Domains Bot**             | Checks for a domain name’s availability. You can search for your desired domain name.                                        
 **Domatron Domains**        | Find available, brandable .com domain names for your business businesses.                                                    
 **DoorDash**                | Discover nearby restaurants for all of your delivery/pickup needs!                                                           
 **Dover Assistant**         | Generate a personalized email to someone you’re interested in reaching out to for a job opportunity.                         
 **Dr. Thoth’s Tarot**       | Tarot card novelty entertainment &amp; analysis, by Mnemosyne Labs.                                                          
 **Draw**                    | Bring your ideas to life with diagrams. Craft, manage, and engage with your creations in a whole new way.                    
 **Dream Interpreter**       | Interprets your dreams using advanced techniques.                                                                            
 **Dream Interpreter**       | Dive deep into the realm of dreams. Uncover their hidden meanings for clarity, growth, and healing.                          
 **Dream Reader**            | Read and decode your dreams and unveil your innermost thoughts and desires.                                                  
 **Drink Maestro**           | Learn to mix any drink you can imagine (real or made-up), and discover new ones. Includes drink images.                      
 **Duoduo English**          | Learn and practice English for Duolingo English Test.                                                                        
 **dxFeed Spotlight**        | Get latest news and DxFeed analytics about companies, sectors and people.                                                    
 **Earnr: UK Tax Helper**    | Earnr provides tax calculators and answers to questions related to UK tax and accounting.                                    
 **Earth**                   | Generates a map image based on provided location, tilt and style.                                                            
 **Earthquake Info**         | Get latest earthquake information.                                                                                           
 **Easy Product Search**     | Easy Product Search simplifies shopping on Japanese EC sites using keywords. It providing product info.                      
 **Easy Resume**             | Quickly create and edit your resume with the option to download as a docx or pdf, and find the job you deserve!              
 **Edmunds Cars**            | Use Edmunds to get trusted car reviews and local new and used vehicle listings.                                              
 **Eduguide**                | A tool designed to respond to college admission-related inquiries.                                                           
 **edX**                     | Find courses and content from leading universities to expand your knowledge at any level.                                    
 **Email by Nylas**          | Connect with any email provider and engage with your email data seamlessly.                                                  
 **Email Me**                | Send yourself email summaries, next steps, and other useful information from your conversations.                             
 **Emotimg**                 | The images(maid character) attached make the conversation more enjoyable.                                                    
 **Empregos Agro**           | Explore the Empregos Agro platform and search for jobs and learning content in the agro-business.                            
 **Engage AI**               | Retrieve hashtag follower counts, trends &amp; suggestions to increase your content reach on LinkedIn.                       
 **Eraser**                  | Generate diagrams from code or natural language.                                                                             
 **eSIM Travel Asst**        | Find eSIM data plans easily! Enter location and eSIM keywords to get apt package suggestions. Ideal for travelers.           
 **Etihad Airline**          | Search flights, and answer other related questions of flights.                                                               
 **Everyprint3D**            | Generate 3D ideas and bring them to life with our expert support.                                                            
 **everysize**               | Search for sneakers in your size and get the best price from 60+ shops (only in DE).                                         
 **ExamenChat**              | Master the Dutch ‘Centraal Eindexamen’: access a vast question library, practice, get hints, and graded feedback.            
 **Exchange Rates**          | Exchange Rates delivers real-time and historical data, enabling conversion and tracking for over 170 currencies.             
 **Exercise Consulting**     | Manage your exercise routine. You can view exercises for different body parts and get details about each exercise.           
 **Expedia**                 | Bring your trip plans to life – get there, stay there, find things to see and do.                                            
 **Experiences**             | Provides activity suggestions, ensuring an engaging and user-friendly travel experiences.                                    
 **Export Chat**             | A Tool to export your conversation or specific parts of your conversation.                                                   
 **Export Chat to PDF**      | I will export your chat conversation to a PDF file, ready for download and share.                                            
 **Express Tracking**        | An assistant for tracking international express logistics.                                                                   
 **FaceTheFacts**            | Access profiles of German politicians, retrieve voting records, analyze side jobs, and monitor party donations.              
 **Fact Fanatic**            | Experience a fusion of interactive facts and captivating quirky idioms to enrich your curiosity-driven exploration.          
 **Factiverse**              | Fact-check your conversations and find credible sources from the web in real time.                                           
 **FakeAGI**                 | Automate your goals: Our AI agents solve tasks and log the results in a Google spreadsheet.                                  
 **FantasyoMatic**           | Your personal FantasyoMatic expert.                                                                                          
 **FCA FS Search**           | Search FCA and get product, individual and firm info using PRN, IRN and FRN.                                                 
 **Federal Resume**          | I’ll write an ATS-optimized professional federal usajobs resume, for your targeted government job role.                      
 **Feng Shui Divination**    | Using traditional Chinese theories such as the I Ching (Book of Changes) to predict the past, present, and future.           
 **Ferryhopper**             | Search for ferries and plan your trip with the assistance of AI and Ferryhopper.                                             
 **Figlet**                  | Utility for converting strings of text into ASCII fonts.                                                                     
 **FilmFindr**               | Tell me your favorite movies and I’ll recommend your next cinematic gem!                                                     
 **Filtir**                  | Verify the provided text using external sources.                                                                             
 **Find a Freelancer**       | Find human freelancers to help complete tasks.                                                                               
 **Find a Tour**             | Find a tour, activity, or experience to do in locations across the world.                                                    
 **Find Talent**             | Find digital freelance talent for any occasion.                                                                              
 **Finna Bolag**             | Seamlessly search for and retrieve Swedish companies’ financial data.                                                        
 **FiscalNote**              | FiscalNote enables access to select market-leading, real-time data sets for legal, political, and regulatory…                
 **FlashcardsGenerator**     | Generate Anki flashcards and downloable Decks!                                                                               
 **Flight Tracker**          | Access real-time airline data, aviation history, and essential travel details instantly.                                     
 **Flowise Support Help**    | A Conversational Agent to ask questions about how to install and use Flowise.                                                
 **For.io App Creator**      | Create feature-rich web apps by simply writing the requirements.                                                             
 **Forex-Rates**             | Forex Rates: Price analysis for currency markets. Gain insights, sentiment evaluation, and text completion.                  
 **Forma Cloud**             | Learn how to drastically reduce your company’s cloud computing costs with the power of our automated solution.               
 **forms.app**               | Generate online forms, surveys, quizzes &amp; more with spot-on questions.                                                   
 **Fortune Cookie**          | Select a number from 1 to 10 and let the digital cookie guide your day!                                                      
 **Foxit PDF Assistant**     | This plugin converts PDF and Office documents interchangeably, supporting formats like Word, Excel, and…                     
 **Free Kiddie Books**       | Free Kiddie Books by www.storybee.space! Type ‘Storybee Free Kiddie Books’ to explore a world of imagination.                
 **FreshTech**               | Fetches and presents the latest tech documentation content.                                                                  
 **FundsDB**                 | Discover funding opportunities in UK and India on FundsDB. Type in your query in any language or /help for                   
 **G2.com**                  | Find the best software and service solutions to your business needs.                                                         
 **Game Box**                | Discover, play, and solve classic games like Blackjack, Tic-Tac-Toe, Word Guess, Sudoku, 24, Yahtzee, and more!              
 **GameBase**                | Chat and get game info, database is based on the latest gaming information in 2023, supports multiple platforms.             
 **GameSight**               | Discover games, game-related content, get recommendations, and compare games based on player reviews.                        
 **GaryVee By Spext**        | Chat with the GaryVee Keynote Speeches Library on Spext.                                                                     
 **Gate2AI**                 | Discover the perfect AI tools for your needs.                                                                                
 **Gerrit for Chromium**     | Search Gerrit Code Review for Google Chrome. Helps analyzing code changes, bugs, features, and more.                         
 **Get Social Proof**        | Create engaging testimonials, customer stories and case studies using AI-powered interviews.                                 
 **GetByBus**                | GetByBus is online travel platform selling bus and ferry tickets for more than 5000 operators.                               
 **GetYourGuide**            | Find tours, excursions and other travel activities.                                                                          
 **GIF Search**              | Search through a wide range of gifs – Powered by Giphy.                                                                      
 **Gift Finder**             | Your personal gift advisor. Find a perfect experience for anyone.                                                            
 **Gift ideas suggester**    | Generate gift ideas for any occasion, such as birthdays, Mother’s Day, etc. Please, provide details about recipient.         
 **GIFT SENSAI**             | Find perfect gifts for friends, family, and colleagues on Amazon.com. Best for birthdays, Christmas &amp; anniversaries.     
 **Giftwrap**                | Ask about gift ideas for any occasion and recipient. Get it wrapped and delivered, no address needed.                        
 **Giga Tutor**              | Giga is your AI powered personalised tutor, it keeps the answers to your questions personalised.                             
 **GigTown**                 | Discover a world of local musical talent and live music performances with the GigTown integration.                           
 **Gimmee Air Quality**      | Planning something outdoors? Get the 2-day air quality forecast for any US zip code.                                         
 **Git OSS Stats**           | Dynamically generate and analyze stats and history for OSS repos and developers.                                             
 **Glamai**                  | The ultimate beauty app to find makeup and skincare that match your needs. Like “a non-sticky sunscreen for traveling”.      
 **GlassBot**                | An interactive plugin for 1-800-glass!                                                                                       
 **Glowing**                 | Schedule and send daily SMS or WhatsApp messages – reminders, inspiration, helpers and more.                                 
 **Go!**                     | Go! for personalized workout plans, fitness tips, and meal recommendations.                                                  
 **GoDaddy Name Search**     | Search and check the availability of domain names using GoDaddy.                                                             
 **GoFynd**                  | Elevate your shopping experience with GoFynd.                                                                                
 **Golden**                  | Get current factual data on companies from the Golden knowledge graph.                                                       
 **GoPlus Security AI**      | Get crypto security information by using GoPlus Security API.                                                                
 **Got2Go**                  | Your next vacation is one conversation away. Literally. Find the perfect stays in the US with Got2Go.                        
 **GoWithGuide**             | Book a private customizable tour with a local guide.                                                                         
 **GrabJobs**                | Your personal job search assistant. Find the best full-time jobs, part-time jobs and internships worldwide.                  
 **Graduate Resume**         | I’ll create you an ATS-friendly recent college/graduate resume, designed to land your first job.                             
 **Graph Constructor**       | A tool for creating spider and bar graphs.                                                                                   
 **HACKIT Web Scanner**      | AI Powered Web Scanner by HACKIT.                                                                                            
 **HackTrack**               | This tool checks if credentials linked to an email have been exposed in data breaches or hacks.                              
 **Hadith Advice**           | Ask a question and get advice from hadith.                                                                                   
 **HaffPrice**               | Shopping all kinds of products with the lowest price in the market.                                                          
 **Hair Care,Simplified**    | Get ideal routines based on your hair’s unique needs and goals.                                                              
 **Halifax Bylaws**          | Your Halifax Bylaw’s Expert.                                                                                                 
 **Hauling Buddies**         | Locate dependable animal transporters using recommendations, reviews, and regulatory compliance search features.             
 **HDB Car Park**            | For checking availability of car park lots at various HDB car parks around Singapore.                                        
 **Hedera Account Info**     | Gets the HBAR and token balance of a Hedera account.                                                                         
 **HeyGen**                  | Meet HeyGen – The best AI video generation platform for your team.                                                           
 **HiCollectors Finder**     | Search and price comparison for eBay products. Only available in the United States.                                          
 **highPerplexity**          | Integrates with highPerplexity and executes any prompts you need.                                                            
 **Horoscope**               | Get near-term readings for your life’s journey. Harness the stars’ wisdom for spiritual guidance.                            
 **Horoscopes**              | Daily, weekly, and monthly horoscopes tailored to you. Brought to you by Inner Self.                                         
 **Hot Pepper Gourmet**      | You can easily find restaurants in Japan listed in “Hot Pepper Gourmet”.                                                     
 **How Long To Beat**        | Find out how long it takes to beat any game!                                                                                 
 **HTML Builder**            | Build, view, and edit HTML &amp; CSS (+ Tailwind) website components with prompts — no code required.                        
 **HTTP Webhooks**           | Allows you to write, deploy, and manage HTTP Webhooks in JavaScript, right from the chat.                                    
 **Hubbub**                  | Local health risk &amp; safety guidance for COVID-19, Flu, RSV and more in the US.                                           
 **HubCart POD Designer**    | Brainstorm print-on-demand design ideas, then make POD designs. Created with DALL·E.                                         
 **Humanize**                | Humanizing AI content via paraphrasing.                                                                                      
 **I Am Rich**               | Proudly declare ‘I am rich’.                                                                                                 
 **I Ching**                 | Unlock ancient I-Ching wisdom: embrace spirituality, ignite creativity &amp; conquer life’s changes!                         
 **Ibotta Shopping**         | Your perfect shopping companion! Search for products from thousands of retailers. Only available in the US.                  
 **idealo Best Price**       | ind the products you need and great savings comparing products and prices from thousands of trusted shops, ads-…             
 **Idea Organiser**          | Organise and store your notes on Notion. Make your note taking smarter, easier and more effective.                           
 **Image Converter**         | Convert an image to any other image format.                                                                                  
 **Image Editor**            | Edit images seamlessly, resize, crop, blur and many more embedded features.                                                  
 **ImageSearch**             | Discover complimentary images to enhance your generated article or to highlight specific paragraphs from                     
 **Imgenic**                 | Generate and evaluate photographic Midjourney image prompts.                                                                 
 **Indoor Plants**           | Trusted Information About Indoor Plants and Gardening.                                                                       
 **IndoorPlantBot**          | The ultimate guide for indoor plants.                                                                                        
 **instabase**               | Search for rooms and venues all across Japan from instabase.                                                                 
 **InfoJobs**                | Search jobs on InfoJobs.                                                                                                     
 **Instacart**               | What’s cookin’? Ask about recipes, meal plans, &amp; more — and get ingredients delivered from 40,000+ stores!               
 **Interest Rates**          | Get Real Time Interest Rates From Central Banks                                                                              
 **Invoice**                 | I’ll create professional good-looking invoices. Download as PDF or Word. Built for small businesses.                         
 **Invoices**                | Create and customize PDF invoices or estimates in seconds.                                                                   
 **io.livecode.ch**          | Evaluate snippets of code using io.livecode.ch.                                                                              
 **IoTHub**                  | IoTHub: Control your Ecobee Thermostats and IoT devices from anywhere.                                                       
 **Isotonix**                | Isotonix products search.                                                                                                    
 **ISS Location**            | Add-on for displaying the current coordinates of the ISS and the names of the current astronauts in space.                   
 **Italy Latest News**       | Get the most recent news from Italy.                                                                                         
 **ixigo**                   | PLAN your trips! Search for best flights &amp; get recommendations where you can travel on your budget.                      
 **James Allen Jewelry**     | Experience the future of diamond and jewelry shopping where stunning quality meets unparalleled customization.               
 **Japan OpenData**          | Search for opendata in Japan, using the search.ckan.jp dataset.                                                              
 **Japanese Strokes**        | Educational tool to help with learning Japanese writing strokes using animations.                                            
 **JetBook.Click**           | Your ultimate travel companion: search/book flights at best prices, get info about your destination. Multilang support.      
 **JiggyBase**               | Use the knowledge in your JiggyBase document collections to help produce factual, up-to-date chat responses.                 
 **Jini**                    | Get factual, knowledge-base and real-time information. Search news, images, videos, music, apps, pages and facts.            
 **Jio**                     | Explore TiraBeauty, JioCinema, JioMart, and JioFiber in one go. Shop, stream, and surf with ease!                            
 **JIRA Issue Helper**       | Create, read, update, and delete JIRA issues.                                                                                
 **JiraTalk**                | Boost Jira’s efficiency via automated project management operations, strategic user insights, and detailed analytics.        
 **Job Cover Letter**        | I’ll make a powerful, ATS-friendly cover letter for your job application. Ideal for email or LinkedIn use.                   
 **Job Description**         | I’ll write you an effective job description that attracts top-quality talent for your next interview.                        
 **Job Interview**           | I’ll prepare you for a job interview by asking questions and providing feedback.                                             
 **Job Offers Copilot**      | I’ll create good-looking PDF job offers for business owners, HR and recruiters to enhance the hiring process.                
 **Job Search**              | Searching for jobs made easy.                                                                                                
 **Job search by Indeed**    | Explore &amp; discover the latest open jobs from the #1 job site in the world, Indeed.com.                                   
 **Job Search UK**           | Get the latest job posts from the UK’s top job boards including Reed, Indeed, and others.                                    
 **jobs.ch job search**      | Find your dream job in Switzerland. 100’000+ job openings await you!                                                         
 **Jobsearch**               | This is a job search service. For now only for jobs in Germany.                                                              
 **Jooii**                   | Chat with jooii to discover millions of products, with personalized recommendations made easy.                               
 **JoPilot**                 | Search for US jobs by keywords, locations, employers, salaries, and commute time.                                            
 **Jot it down**             | Remember details from your chats by jotting them down. You can add, remove and recall your ‘jots.’                           
 **K-Fashion**               | You can search for the product you need among the products sold by the stores in CAFE24.                                     
 **Kakaku.com**              | Search for products that match your criteria from among the many products registered on the Japanese website…                
 **Kakaku.com/travel**       | You can search for hotel that match your search criteria from among the many hotels registered on Kakaku.com.                
 **Kakuyasu Idou**           | Compare prices for express bus, flight and bullet train when traveling in Japan. Offer the best for booking!                 
 **KalendarAI**              | KalendarAI sales agents generate revenue with potential customers from 200+ million companies globally.                      
 **Kargo Takibi**            | Track where your the parcel is, its movements and more for Turkish cargo providers.                                          
 **KAYAK**                   | Search flights, stays &amp; rental cars or get recommendations where you can go on your budget.                              
 **KeyMate.AI Search**       | Search&amp;Browse the web by using Google Search results with KeyMate.AI, your AI-powered web crawler.                       
 **Keyplays Live Soccer**    | Latest live soccer standings, results, commentary, tv stations, keyplays (with and without scores).                          
 **Keyword Explorer**        | Keyword Explorer provides popular related keywords to amplify your content optimization.                                     
 **Kids’ Stories**           | Kid’s Personalized Story Maker: Input names &amp; traits for a unique tale! Choose length &amp; age for a perfect fit.       
 **Kiwi.com**                | We hack the system, you fly for less. Book cheap flights other sites simply can’t find.                                      
 **Klarna Shopping**         | Search and compare prices from thousands of online shops. Only available in the US.                                          
 **KlickRx**                 | KlickRx connects to MedOcean, Klick’s proprietary database. Search payments by industry to US doctors using their…           
 **Klook**                   | From local escapes to far flung adventures, find the best experiences, tours, hotels and transport options anywhere.         
 **Kokoro**                  | Learn with Kokoro!                                                                                                           
 **Konkani Translator**      | Unlock the Beauty of Goa with the Romi Konkani Translator.                                                                   
 **KonnectzIT**              | Integrate to connect &amp; automate your favourite Marketing, CRM, Payments and many more applications.                      
 **Korea subway route**      | Korea metro subway route info.                                                                                               
 **Kraftful**                | Product dev coach made by kraftful.com. Ask about best practices. Get top gurus’ product thinking.                           
 **Kurumaerabi.com**         | We suggest the car you want from the used cars registered in www.kurumaerabi.com.                                            
 **Kyujinbox**               | Searching jobs in Japan. You can search jobs by keyword, location and employ type.                                           
 **Lawyer Search (CA)**      | Search for lawyers by name, specialty, language, and City. Currently limited to California, USA.                             
 **LawyerPR**                | Matching your ideal lawyer, in Japan. Let’s Start with a Preliminary Review.                                                 
 **LegalQA**                 | Answer any inquiries related to Chinese law in any languages.                                                                
 **Lei Brasileira**          | Updates on Brazilian legislation and information about the operation of the Legal Briefing solution.                         
 **LevinBot**                | Search research papers and transcripts of talks from Dr. Michael Levin’s Lab.                                                
 **Lexi Shopper**            | Get product recommendations from your local Amazon store.                                                                    
 **LGTM**                    | Search for LGTM Markdown.                                                                                                    
 **Likewise**                | Your media and entertainment companion. Get TV, Movies, Books &amp; Podcast Recommendations.                                 
 **LincolnBot**              | The ultimate trusted guide for all things related to Abraham Lincoln.                                                        
 **Lingo**                   | Lingo – Direct Access to the Japanese Lifestyle with One Click.                                                              
 **Link Analyzer**           | Analyze a link for safety and security.                                                                                      
 **Link Reader**             | Read any links: webpage, youtube, PDF, PPT, image, Word doc etc; OCR image; Search &amp; browse web.                         
 **Linkhouse**               | Linkhouse builds a ready-to-buy list of backlinks with their SEO statistics tailored to your domain.                         
 **Listen Notes**            | The best podcast search engine and database. All podcasts and episodes. Built with PodcastAPI.com.                           
 **Litmaps**                 | Get help exploring the scientific literature. Find relevant papers and generate mindmaps of the literature.                  
 **Little Professor**        | Little Professor creates student tests for classroom teachers to enhance school education.                                   
 **Live Game**               | Get real-time scores and news updates from the world of eSports. LOL, Dota2, CS:GO, PUBG live scores and news.               
 **liveinboxer**             | Get the latest email from top brands.                                                                                        
 **Local by GoodCall**       | Discover and support restaurants, shops &amp; services near you. 🍽️ 🛍️ 🔧                                                  
 **Love Coupons**            | Find money saving coupon &amp; discount codes for over 30,000 stores around the world.                                       
 **Lsong AI**                | Lsong’s AI provides AI-powered content like news, images, music, movies, weather, stories, memes, and more.                  
 **Lucas AI Video**          | Turn any response into a useful video in seconds! Plus, easily edit and customize your video.                                
 **Lucid**                   | Easily clarify complexity when you create and edit diagrams in chat.                                                         
 **MacroMicro**              | Search the economic stats or ETF from MacroMicro.me and get newest value.                                                    
 **Magic**                   | Assign tasks to a 24/7 on-demand remote worker with 1-click using Magic.                                                     
 **Magic Conch Shell**       | Guide your most important life decisions with the magic conch shell. (Please don’t actually, it’s only satire).              
 **MagiCodex**               | Ask about Magic: The Gathering cards, rules and interactions.                                                                
 **MagicSlides.app**         | MagicSlides.app: Ideas to Professional Presentations in Seconds.                                                             
 **Magnetis**                | Magnetis is a digital wealth manager. Get updated data on portfolios returns and allocations. Ask me about Magnetis.         
 **Maibook**                 | Find Korean books to read. Just type in what you’re in the mood to read—no need for the title or name of the author.         
 **Maimovie**                | Maimovie, the AI-powered movie search, understands any conversational prompt like “movies with plot twists”, “tyrant”        
 **Make A Sheet**            | Generate a csv file that can directly be imported into Google Sheets or MS Excel.                                            
 **Man of Many**             | Discover the latest in products, culture and style from Man of Many. Ask for the latest news and headlines.                  
 **Mantium**                 | Fetches user-created applications from Mantium and retrieves relevant info based on user queries.                            
 **Maps**                    | A Maps app that takes latitude and longitude coordinates and generates a map with a marker on the given spot.                
 **Maps Pro**                | Create and style maps with markers and paths from both coordinates and addresses.                                            
 **Math Tutor**              | Share your calc, stats or any other math homework, receive guided answers to aid your learning.                              
 **MBTI**                    | For administering an MBTI test. You can get a list of questions and calculate your MBTI type.                                
 **MC Expert**               | MC Expert provides guidance on Minecraft command, plugin, and Skript development.                                            
 **Meal Planner**            | Craft diets to fit your tastes. Explore diverse food selections, cook with flair, and enhance your gastronomic journey!      
 **Medecine-Explorer**       | Facilitates access and search for Medicine articles, including author and journal details – By Nabil Brag.                   
 **Medical Device EU**       | Discover all relevant Guidances, Regulations and Requirements in the European Medical Devices field.                         
 **Medical Resume**          | I’ll create an ATS-optimized job winning professional medical assistant resume for your targeted healthcare position.        
 **Meme Creator**            | Use Meme Creator to create memes on demand using the power of AI!                                                            
 **Meme Generator**          | Your AI meme generator.                                                                                                      
 **Memory Bank**             | Extend AI memory. Ask it to store and retrieve your information to get personalized assistance tailored to your needs.       
 **Mercari**                 | Explore a variety of products on Mercari, Japan’s largest C2C marketplace.                                                   
 **Mermaid Chart**           | Visualize Mermaid Diagrams and fine-tune them in the Mermaid Chart editor.                                                   
 **Message in a Bottle**     | Throw a message into the digital ocean, or grab one floating by. You are not alone.                                          
 **MetaMentor by AxonAI**    | MetaMentor personalizes your learning path in any topic and provides you with PDF,HTML and DOCX study guides.                
 **Metaphor Search**         | Access the internet’s highest quality content. Recommended by people, powered by neural search.                              
 **Michelangelo**            | Create high quality images based on different art styles by leveraging the power of generative AI using DALL-E 2.            
 **midpage caselaw**         | Interact with US caselaw: Searches, summarizes, answers, and checks legal statements.                                        
 **Mifuzi Domain AI**        | Discover domain names based on your creative ideas and check their availability status.                                      
 **MightyOwl K-8**           | MightyOwl is a K-8 educational resource offering fun and engaging videos, quizzes and worksheets.                            
 **Million Dollar Ads**      | They stacked a million dollars to catch your eye! Browse the gallery of marketing masterpieces.                              
 **Mindart**                 | Career test to help you find your dream job, with automation risk and average salary.                                        
 **MindfulDataAI Search**    | Google-powered web searches with ChatGPT. Provides up-to-date information on recent events, trending topics, and more.       
 **Minecraft Chocolate**     | Ask for recomendations about Minecraft mods and modpacks. Uses Modrinth API.                                                 
 **Mini Games**              | Play text based games in your chat! Hangman, tictactoe, adventures, or make your own!                                        
 **Mini Habits**             | Form new habits through small, daily actions.                                                                                
 **Mino**                    | Discover the easiest way to find the best coupons for thousands of online stores.                                            
 **MintbaseAction**          | Submit transactions to the NEAR Protocol. It only works on Testnet. Use this for testing purposes only.                      
 **MintbaseSearch**          | Find any NFT data on the NEAR Protocol.                                                                                      
 **20 Minuten (CH) News**    | Get the latest News from 20 Minuten about Switzerland and the world.                                                         
 **MixerBox Calculator**     | Your digital math assistant for precise calculations! Accuracy and efficiency in one place!                                  
 **MixerBox Calendar**       | The best Google Calendar AI assistant! Master your schedule easily! Create events and set reminders!                         
 **MixerBox ChatDrive**      | The best Google Drive AI assistant! Easily manage any files: PDFs, PPTs, images, docs, sheets, etc.                          
 **MixerBox ChatMap**        | Powered by Google Maps API, MixerBox ChatMap is the world’s 1st AI chatbot for Maps!                                         
 **MixerBox ChatPDF**        | Save time! MixerBox ChatPDF can summarize &amp; analyze PDF files for you!                                                   
 **MixerBox ChatVideo**      | Summarize videos from YouTube! Ask questions and get answers right away!                                                     
 **MixerBox Diagrams**       | Create, edit, and showcase diagrams through chat! Make presenting your data incredibly simple!                               
 **MixerBox FindPlugin**     | Find the plugins you need instantly! Help you quickly accomplish tasks and save a lot of time!                               
 **MixerBox FreecableTV**    | Watch free ad-supported TV shows, series, live channels, movies, news &amp; sports from across the web!                      
 **MixerBox ImageGen**       | Generate AI Images via DALL·E 2! Create perfect prompts for Midjourney &amp; Stable Diffusion pictures!                      
 **MixerBox News**           | Get the latest news with ease! Search and browse updates on finance, sports, technologies and more!                          
 **MixerBox OnePlayer**      | Unlimited music, podcasts, and videos across various genres. Enjoy endless listening with our rich playlists!                
 **MixerBox PhotoMagic**     | Enhance photo resolution magically! Achieve clearer &amp; more detailed visuals for your images!                             
 **MixerBox Podcasts**       | Search podcasts easily! Explore podcasts covering society, sports, business, news, music, and more!                          
 **MixerBox Prompt Pro**     | Generate ideal prompts instantly! Ensure accurate and comprehensive responses effortlessly!                                  
 **MixerBox QR**             | Create stunning AI-generated QR code images instantly! Make your QR code leave a lasting impression!                         
 **MixerBox Scholar**        | Free and reliable academic search engine! Find research papers and get answers in an instant!                                
 **MixerBox Translate**      | Translate any language right away! Learn foreign languages easily by conversing with AI tutors!                              
 **MixerBox Weather**        | Get real-time worldwide weather updates &amp; forecasts, instantly!                                                          
 **MixerBox WebSearchG**     | Search and summarize the web with our customized search engine powered by Google Search API!                                 
 **ML Paper Reader**         | Search for ML papers on different topics and speed up research by “talking” to the PDFs.                                     
 **MLB Stats**               | Access up-to-date baseball news and statistics for players and teams across the MLB.                                         
 **Mobitrix**                | Discover Info for All Smartphones, Tablets, and Watches.                                                                     
 **Mobula**                  | Fetching real-time data for all crypto &amp; blockchain metrics.                                                             
 **Mojito**                  | I can convert your chats to articles and posts them online🍹.                                                                
 **Moment Of Zen Spe**       | A knowledge retrieval system for the Moment Of Zen podcast in Spext.                                                         
 **MomentX GuideX**          | It gives real-time info on our store, including brand locations and promotions.                                              
 **Monarch Initiative**      | Search and retrieve basic information from the Monarch Initiative knowledge graph.                                           
 **Moodo**                   | Moodo® is one of a kind aroma diffuser, that can mix up to 4 scents. Moodo version 3+ allows air purifying too.              
 **Movie Night**             | Pick movies your group will love. Simple and fun!                                                                            
 **Movies Detective**        | Persisting movie preferences and giving recommendations. You can add movie descriptions or ask to get all of them.           
 **MpcWallet**               | Unleashing Blockchain Transparency: Your Tool for Deep Dives into DeFi, NFTs, Tokens, and the Web3 Landscape.                
 **MultiSheet**              | Generate Excel sheets (XLSX) with multiple tabs effortlessly and Securely.                                                   
 **Murder Mystery**          | Unleash your inner detective! Choose era, theme, victim &amp; challenge. Solve your own crime!                               
 **Music**                   | Generate music, compose music, create music and listen to music. Save music and refer it in the future.                      
 **My Writing Companion**    | Find, hire, and manage remote human writers, the best way to ensure your content is engaging, accurate, and error-free.      
 **Mybest**                  | Find out which products are best for you in Japan. You can ask for recommended products.                                     
 **MySkinRoutine**           | Crafting the perfect care routine for your unique skin.                                                                      
 **myTomorrows Search**      | Discover and filter clinical trials worldwide; updated daily.                                                                
 **NamePedia**               | Behind every name is a story: uncover origins, trends, and famous bearers.                                                   
 **Namr.pro**                | Say “namr” to find awesome domains for your personal or professional brand.                                                  
 **NASA Media Explorer**     | Discover and learn about space exploration using NASA’s vast media library!                                                  
 **ndricks Sports**          | Get information about pro teams (NHL, NBA, NFL, MLB) teams by calling the ndricks Software Sports API.                       
 **ne.tools**                | Network diagnostic tool for DNS record lookup (A, AA, MX, NS and more) and WHOIS queries for domains and IP addresses.       
 **Netlify Drop**            | Describe a simple website you want to make, and deploy it to Netlify to share it with others and claim it as your own.       
 **Newegg PC Builder**       | Build and purchase a custom gaming PC based on your budget and needs.                                                        
 **News**                    | The most popular global news app, digest 80,000+ trusted sources for the latest news, trends, and insights in a flash!       
 **NewsBreak**               | Learn about the most popular local or national news and find local businesses and events in the United States.               
 **Newsimoji**               | A fun game where you guess a news story based on solely on emojis.                                                           
 **NewsPilot**               | Breaking global news in brief, 15000 news sources from worldwide.                                                            
 **NextPaper.ai**            | Fetch the latest research papers on a specific topic from PubMed. More to come.                                              
 **NFT Guru**                | Discover current prices of NFTs across major platforms and keep track of the rapidly changing marketplace with real-time     
 **NFT News Roundup**        | Get Today’s NFT News Headlines As A Clickable Link Roundup!                                                                  
 **NFTs**                    | Get the important details of any NFT collection and ask for insights based on that data!                                     
 **NJSI Beta by SSG**        | National Jobs Skills Intelligence. Skillsfuture Singapore AI Graph for insights and relationships in the JS                  
 **Nominus.com**             | Check if your domain name is available. We support all ccTLDs and gTLDs.                                                     
 **Noteable**                | Create notebooks in Python, SQL, and Markdown to explore data, visualize, and share notebooks with everyone.                 
 **NotesAISync**             | Unofficial connector to Notion for interactive note-taking.                                                                  
 **Now**                     | Get Google Trends. In Japan, you can also get Twitter trends and search Twitter keywords.                                    
 **Numerology**              | Unleash the power of numbers. In seconds, find guidance and understand your true spiritual self.                             
 **OfferZen World**          | Queries OfferZen’s database for tech career, recruitment, service, and pay insights.                                         
 **OhMyPlug**                | Search and discover extensions with OhMyPlug! It guides you through the best addons in the store and explains…               
 **Okappy**                  | Get information about your jobs and engineers. Reassign jobs and create new jobs.                                            
 **Omio**                    | See schedules and book on Omio. Be moved!                                                                                    
 **One Word Domains**        | Check the availability of a domain and compare prices across different registrars.                                           
 **OnePage Stock Ideas**     | Find a list of stock ideas by asking it or feeding it with news or article.                                                  
 **OnlyTriviaUp**            | Answer trivia quizzes and ‘only go up’ in this interactive learning game as you climb a staircase of knowledge.              
 **Open Archives**           | Search the genealogical data of Dutch and Belgian archives and societies via Open Archives.                                  
 **Open Lecture**            | Discover and access the right moments in open course lectures for targeted learning.                                         
 **Open Prompt Library**     | Use prompts from an open database! Create prompts and search by title or prompt similarity. List the top prompts.            
 **Open Trivia**             | Get trivia questions from various categories and difficulty levels.                                                          
 **OpenTable**               | Provides restaurant recommendations, with a direct link to book.                                                             
 **OpenTools AI**            | Find the right AI tools for your needs from the largest collection on the web.                                               
 **Options Pro**             | Options Pro is your personal options trading assistant to help you navigate market conditions.                               
 **Outrank Article**         | Outrank the competition with an in-depth, SEO-optimized article based on [COMPETITOR URL]. Beat them at their…               
 **Outreach**                | Craft a personalized sales email for a potential client you’d like to approach about your services.                          
 **Outreach AI**             | Generate a personalized sales email for a prospect to whom you want to reach out to for selling your service.                
 **Outschool**               | Search for top-quality online classes and teachers on Outschool.                                                             
 **OWIS**                    | Explore the capabilities of the OWIS system for seamless workflow and document management.                                   
 **Owler**                   | Owler provides real-time business news and insights on private and public companies.                                         
 **OwlJourney**              | Provides lodging and activity suggestions, ensuring an engaging and user-friendly journey.                                   
 **P/E For NFTs**            | Get the price to earnings ratio for any NFT collection!                                                                      
 **Page Whisperer**          | Generate content for your web page.                                                                                          
 **Pal**                     | Post interests and values to connect with like-minded friends.                                                               
 **PaperChat**               | Search through arXiv publications.                                                                                           
 **Paperpile**               | Supercharge your research: search millions of scientific papers in seconds.                                                  
 **Papr Memory**             | Retrieve information from your personal memory includes past conversations.                                                  
 **Paraphraser**             | Say it better, say it different: Paraphrase like a Pro. Paraphrase for impact and inspire.                                   
 **PartSecure**              | Search &amp; compare electronic component inventory, pricing, &amp; lead time from top suppliers.                            
 **Party Planner**           | Plan the perfect party.                                                                                                      
 **Passive DNS Pivots**      | ZETAlytics Global pDNS with 800 Billion records. Unearth nefarious domains within minutes of creation.                       
 **Paxi AI**                 | Let AI tools speed up your tasks! Make it easy to find the right tool for the task.                                          
 **PayPay gourmet**          | You can find restaurants in Japan from PayPay gourmet in Japanese. Output may differ from PayPay gourmet.                    
 **PDF Creator**             | Create a PDF file in seconds.                                                                                                
 **PDF Reader**              | Access and chat with PDF links. This allows you to ask questions, summarize, simplify, or rewrite the PDF content.           
 **Peel Hunt AI-ERA**        | AI Equity Research Assistant (AI-ERA).                                                                                       
 **Penrose Analyst**         | Search global news and research papers. Summarize Arxiv.org links. Ask me for the latest news!                               
 **Perfect Chirp**           | Turn your ideas into banger tweets in less than 30 seconds. Start your prompt with ‘Write a tweet…’.                         
 **Persona**                 | Dive into simulated chat adventures with your beloved heroes, experts, and celebs!.                                          
 **Photorealistic**          | Generate Photorealistic prompts for Midjourney.                                                                              
 **Piano**                   | Learn and play piano with the world’s smartest musician.                                                                     
 **Pictory.ai**              | Script-to-Video: Effortless, quick, and vivid storytelling.                                                                  
 **Ping Task Assistant**     | Add your own TODOs, plan and delegate tasks. Automatically assign projects and people.                                       
 **Pixellow**                | Pixellow: Unveiling image insights, aiding understanding, and creating detailed captions and descriptions.                   
 **Placid.app**              | A design assistant that creates marketing visuals from your templates.                                                       
 **Planfit**                 | Get your tailored workout plan and instructions with videos – AI-powered Workout Coach, Planfit.                             
 **Playlist Follow**         | Create and find the best music playlists, all in one place.                                                                  
 **PlaylistAI: AMZN**        | PlaylistAI is an app for generating Amazon Music playlists. Not affiliated with Amazon Music.                                
 **PlaylistAI: Deezer**      | PlaylistAI is an app for generating and remixing Deezer music playlists. Not affiliated with Deezer.                         
 **PlaylistAI: Spotify**     | PlaylistAI is an app for generating Spotify music playlists. Not affiliated with Spotify.                                    
 **Plooral**                 | Explore the Plooral Eduployment platform and search for jobs and learning content.                                           
 **PlugFinder**              | Describe the addin you want, and we will help you find the most suitable one.                                                
 **Pluginpedia**             | Recommend plugins in the store based on your question, and introduce their usage.                                            
 **Plus Recipe &amp; Food**  | Explore recipes with your ingredients! This plugin offers cooking guides and similar dish ideas for culinary                 
 **Podcast Search**          | This tool explores podcasts from PodcastIndex.org, a platform for decentralized audio content discovery.                     
 **PointsRecap**             | Daily stock market updates, trading insights and view points summary from YouTube traders.                                   
 **Polarr**                  | Search Polarr’s massive pool of user generated filters to make your photos and videos perfect.                               
 **Poll the People**         | The ultimate guide for market research and surveys.                                                                          
 **Polygon**                 | Market data, news, and fundamentals for stocks, options, forex, and crypto from Polygon.io.                                  
 **Popfun AI**               | Personalized toy recommendations, ideas and gifts for kids.                                                                  
 **PortfolioMeta**           | Analyze stocks and get comprehensive real-time investment data and analytics.                                                
 **PortfolioPilot**          | Your AI investing guide: portfolio assessment, recommendations, answers to all finance questions.                            
 **PortfoliosLab**           | Stocks, ETFs, funds, crypto analysis: historical performance, volatility, risk metrics, Sharpe ratio, drawdowns, etc.        
 **Power Automate**          | Create and run automated workflows that connect to over 1000 apps and services (Preview).                                    
 **Power Dynamics 365**      | Expert answers on Microsoft Dynamics 365 and Power Platform.                                                                 
 **PPC – StoreYa.com**       | Your personal assistance for automating advertising – Google Ads (AdWords) and Microsoft Ads (Bing).                         
 **Prayer Times**            | Get Prayer Times for all over world in seconds.                                                                              
 **Preply**                  | Finding the best language tutors.                                                                                            
 **Present It: ShowME**      | PPTX Presentation generator, create slides easily! Generates a downloadable document.                                        
 **PrimeLoupe**              | Simplify your decision-making process by summarizing Amazon product reviews for you.                                         
 **Printify**                | Convert your ideas into personalized t-shirts and start your own ecommerce business with Printify.                           
 **ProApp Learn Design**     | Level up your design skills quickly with a wide range of design courses, interactive workshops and AI-guided mentorship.     
 **ProdigiLink Video**       | A ProdigiLink service that allows you to search for and summarize YouTube videos.                                            
 **Prof G Markets Spext**    | Chat with the Prof G Markets Podcast on Spext.                                                                               
 **Project Plan 365**        | Create simple project plans that open directly in Project Plan 365 or export to MS Project’s MPP format for free!            
 **Prompt Enhancer**         | Use AISEO prompt generator for perfect prompts. Use the word AISEO to let the AI generate the perfect LLM prompt.            
 **Prompt Hotkey**           | Don’t waste your time repeating prompts. Create your custom hotkeys and just type ‘hotkey’.                                  
 **Prompt Perfect**          | Start your prompt with ‘perfect’ to craft the perfect prompt, every time.                                                    
 **Promptest**               | Craft better prompts using /enhance, /feedback before a prompt. Skill up with /brainstorm, /workout.                         
 **PromptRecruit**           | PromptRecruit gives you the ability to talk with your recruitment system!                                                    
 **PTCG price research**     | It fetches the latest prices of Pokémon Cards within Japan.                                                                  
 **PubMed Research**         | The most powerful PubMed search tool, powered by Google Custom Search! Find research articles swiftly and easily.            
 **PuginAI**                 | Recommend extensions based on prompt from user.                                                                              
 **Puzzle Constructor**      | A tool for creating crosswords. You can create crosswords from words and hints.                                              
 **PWAD Podcast Spext**      | Chat with the Positioning with April Dunford Podcast library on Spext.                                                       
 **QEEQ**                    | Our mission is to make journeys more enjoyable and bring better travel experiences to QEEQ with the help of                  
 **QR Codes**                | Create QR codes.                                                                                                             
 **QR Generator**            | Generate QR code in seconds.                                                                                                 
 **QS World Uni Ranking**    | Research the performance of universities across the globe using QS World Uni Rankings 2024.                                  
 **QuakePH**                 | Stay updated with the latest earthquakes in the Philippines.                                                                 
 **Quantum Ready Check**     | Check if a website is using quantum-safe key exchange. This helps ensure your website is quantum ready.                      
 **Questmate Forms**         | Create forms, checklists and workflows (we call ’em Quests!) that you can assign, schedule or make public.                   
 **Quick News Digest**       | Discover world news tailored for you. Curated articles aligned with your interests on demand.                                
 **QuickCreator Marketi**    | The generated Google campaign includes Campaign, AdGroup, Ad, and Keywords.                                                  
 **QuickPage**               | Boost your marketing with fast, SEO-optimized landing pages.                                                                 
 **QuickRecall**             | Create flashcards and review them with spaced repeition.                                                                     
 **QuickSEO**                | Get a quick SEO audit for a specific URL.                                                                                    
 **QuickURL**                | Shorten your links and track clicks on them.                                                                                 
 **QuickVoice**              | Get your text converted to audio quickly. Supports over 100 languages ​​and 300+ speakers.                                   
 **Quiver Quantitative**     | Access data on congressional stock trading, lobbying, insider trading, and proposed legislation.                             
 **Quizizz**                 | Gamify your text quizzes and content to drive engagement for assessment, instruction, and practice.                          
 **QyrusTestPilot**          | You can test your webapp without writing a single line of code by just giving the URL                                        
 **Random Generator**        | Generate truly random numbers from atmospheric noise, supported by random.org (not official).                                
 **RCS_AI**                  | Answers users’ questions regarding a website, a webpage or a document such as a pdf.                                         
 **Receipt Assistant**       | Manage receipts with Sparrow. You can read, store and review your receipts.                                                  
 **Recipe Finder**           | Explore meals for all diets: vegan, keto, gluten-free. Dive into curated foods and ingredients. Elevate your cooking.        
 **Recombinant AI™**         | Input a Github repo URL. Get a holistic,deep, relational understanding of the codebase.                                      
 **Recommend OP**            | Add 'RECOP' to your prompt to get relevant OpenPlugin-supported plugins recommended with sandboxes.                          
 **Recruiter. Interview**    | I’ll help recruiters, employers, and business owners prepare for job interviews to enhance the hiring process.               
 **Redesign Resume/CV**      | I’ll redesign your resume or CV, optimize for ATS and tailor it to your targeted job role.                                   
 **Reflect Notes**           | Creates a Reflect note.                                                                                                      
 **Reminders**               | Add, remove, list and tag reminders.                                                                                         
 **Repo Inspector**          | Inspect Git Repositories. Submit a GitHub, Gitlab, etc., HTTPS link. The repo will be reviewed by Repo Inspector.            
 **Repo Radar**              | Your one-stop shop for up to date Github repo information. Find repos by topic, language, or name.                           
 **Research Butler**         | Get a summarized list of research articles based on an any topic.                                                            
 **Research By Vector**      | Unearth precise academic research effortlessly with the power of vector embeddings for relevance and accuracy.               
 **Responsive**              | Take screenshots of any website running on all devices and browsers you want !                                               
 **Resume**                  | Enhance your resume with AI. Submit your PDF URL and ask for quick, AI-guided feedback.                                      
 **Resume Copilot**          | I’ll perfect your resume for ATS, tailor it to the job, ensuring you stand out to recruiters                                 
 **Resume Genie**            | Resume creation made simple. Easily generate an attractive resume. Tailor it further by adding the job post                  
 **Revelo Talent**           | Search through Revelo’s Talent Database to match with the best Software Engineers according to your…                         
 **ReviewReader**            | Tired of comparing countless Amazon reviews? Input a product name or link for an instant AI summary.                         
 **RiskmonsterCorpD**        | Searching for Japanese corporations from Riskmonster’s own database of 5 million companies in Japan.                         
 **RoboAd**                  | Start with a URL and get ad copies for your Google and Microsoft Ad Campaigns in seconds!                                    
 **Rogo**                    | Answers questions about open-source repositories.                                                                            
 **Roshi**                   | Create interactive lessons from online articles or Youtube videos.                                                           
 **RPAMAKER**                | Run and deploy ROBOTFRAMEWORK scripts to RPAMAKER.                                                                           
 **RPP by Spext**            | Chat with the Guy Kawasaki’s Remarkable People Podcast library on Spext.                                                     
 **Runes**                   | Unveil the mystic power of runes for profound insights to guide your personal and spiritual journey.                         
 **RVshare**                 | Plan a seamless RV trip by receiving personalized recommendations for an effortless experience.                              
 **S3 Storage**              | Download, search, and upload files to private buckets on any S3 API compatible service.                                      
 **SA Speed Cameras**        | See if a mobile speed camera or roadwork is on a South Australian road today!                                                
 **Safe to run**             | Interact directly with your safe to run account.                                                                             
 **Sahih AI**                | Query a list of islamic hadiths from our prophet Muhammed (PBUH)’s sunnah based on the 6 sahih hadith books                  
 **Sakenowa**                | Find Sake and get detailed information in various ways.                                                                      
 **Saltwire Search**         | Interact with the news instead of just reading it.                                                                           
 **Save**                    | Save articles, data, ideas and research findings. It is an efficient way to access this stored content in the future.        
 **Savvy Trader AI**         | Realtime stock, crypto and other investment data.                                                                            
 **SceneXplain**             | SceneXplain lets you attach images to your prompt. Explore image storytelling beyond pixels.                                 
 **Scholar Assist**          | Search academic research papers from arXiv and find answers to your questions.                                               
 **ScholarAI**               | Unleash scientific research: search 40M+ peer-reviewed papers, explore scientific PDFs, and save to reference managers.      
 **Scholarly**               | Scholarly is an AI-powered search engine for exploring scientific literature.                                                
 **Scholarly Graph Link**    | You can search papers, authors, datasets and software. It has access to Figshare, Arxiv, and many others.                    
 **ScholarlyInsight**        | Query research papers from Arxiv.                                                                                            
 **School Supply List**      | Your ultimate destination for school supply lists across the United States for students from k to 12th grade.                
 **Science**                 | Search over 250M scientific papers and research articles. Perfect for researchers or students.                               
 **Scrapee**                 | Scrape the text content of any website.                                                                                      
 **Scraper**                 | Scrape content from webpages by providing a URL.                                                                             
 **ScrapingBee**             | Search Google for finding general information and urls. Or scrape websites for the most real-time specific                   
 **Search UK Companies**     | Fetching public information on UK registered Companies and it’s Officers from Companies House.                               
 **Seats.aero**              | Instantly discover flights that can be booked with points and miles.                                                         
 **60sec site**              | Generate a beautiful website in 60 seconds using AI.                                                                         
 **Secretary Kim**           | Korean weather, subway information! Enjoy a smart life with Secretary Kim.                                                   
 **Sembot**                  | Manage your Search Engine Marketing. Get keywords reports, CPCs, domain visibility, SEO results and more.                    
 **3 Sentence Service**      | Managing a three sentence service. You can add, remove, view and invoke your 3 sentence services.                            
 **Sentence Beasts**         | Summon or create unique word monsters, engage them in thrilling battles, and record the outcomes using Word Monsters.        
 **Sentiment Analysis**      | Sentiment Analysis.                                                                                                          
 **SEO**                     | Enter any URL and keyword and get an On-Page SEO analysis &amp; insights!                                                    
 **SEO Assistant**           | The SEO Assistant can generate search engine keyword information in order to aid the creation of content.                    
 **SEO Audit**               | Technical SEO report and website audit tool. Check your webpage health for SEO rankings.                                     
 **SEO Blog Writer**         | Generate Quality, Human-like, SEO-Optimized Multilingual Blogs &amp; Publish Instantly to WordPress!                         
 **SEO Content Marketi**     | SEO tool for your Shopify store, ecommerce website or any business. Create strategy, blogs and socia.                        
 **SEO CORE AI**             | Use AI to analyze and improve the SEO of a website. Get advice on websites, keywords and competitors.                        
 **SEO Score Rate**          | Guide Your Content Quality with Google E-E-A-T: Continuously Elevate Your SEO Ranking!                                       
 **SEO.app**                 | Your personal SEO assistant for content marketing.                                                                           
 **Serpstat SEO Tool**       | Discover SEO keywords for a specific topic and analyze SEO metrics/backlink data for a domain with Serpstat.com.             
 **Service Check**           | Check if services are running and how fast they are responding. You can check Website, Email, Domain Lookup and Ping.        
 **Sesotec**                 | Sesotec answer your questions about Sesotec products and services.                                                           
 **SG Parking**              | Offers real-time HDB carpark availability in Singapore. Search by address for nearby parking space info.                     
 **Shein Discovery**         | Get your fashion and lifestyle recommendations from Shein.                                                                   
 **Shop**                    | Search for millions of products from the world’s greatest brands.                                                            
 **Shop AIssistant**         | A shopping assistant helping with the search through a large product catalog.                                                
 **Shop Best**               | Shop and get summarized reviews for the best products on Amazon.                                                             
 **Shop Rewards**            | Amazon product shopping search, conveniently query products, get discounts and discounted products more quickly.             
 **SHOP.COM**                | SHOP.COM Website Search for products, brands, stores and more. Great to find products, great deals, and earn rewards.        
 **Shopping Finder**         | EBay product shopping search, conveniently query products, get discounts and discounted products more quickly.               
 **Shopping tools**          | Shopping tools let you search for products to buy in AliExpress and eBay.                                                    
 **Shor in QASM**            | Generate quantum circuits for Shor’s algorithm in QASM format.                                                               
 **Short Video Trends**      | Find top keywords for short video content. Boost your social media strategy with targeted insights.                          
 **shownotes**               | Turns long podcasts into quick summaries, finds specific info, highlights key points, and suggests new                       
 **Shuto.IO**                | Shuto.IO is a multi-tool for creators and developers with SMS, Email, WordPress and SSH Command Execution capabilities.      
 **sic**                     | Your gateway to crypto. Explore prices, accounts, and transactions on blockchains, starting with Ethereum.                   
 **SID**                     | Connect data from your Google Drive, Notion, and Gmail.                                                                      
 **SightGo**                 | Explore any location with a visual street view perspective.                                                                  
 **SignalPlus**              | Live data on crypto, NFTs, DeFi. Spot/derivatives. A must for traders.                                                       
 **SigTech**                 | Retrieve financial data and test your trading strategies using the SigTech API (learn.sigtech.com).                          
 **SimplyCodes**             | The smartest shopping tool. Save w/ coupons, deals, promo codes at 300K retailers, ecommerce stores                          
 **SingleStore**             | Query your SingleStore database in English.                                                                                  
 **Skrive**                  | Envision your concepts through Diagrams. This tool allows you to create and modify diagrams within the chat interface.       
 **SkyPoint AI**             | Chat with your unified business data to answer questions, automate tasks, and boost your productivity.                       
 **Skyscanner**              | Skyscanner makes planning your next trip easy. Search flight deals and plan your next exciting trip.                         
 **Sleek Choice**            | Save time! Let’s try effortlessly discover top-rated items within a specified budget range on Amazon.                        
 **Slide Maker**             | Create beautiful PowerPoint presentations. Customize and export to PPTX, and more.                                           
 **Smart Connections**       | Chat with your notes integration for Obsidian.                                                                               
 **Smart Slides**            | Smart Slides: AI-powered tool for rapid PowerPoint presentations. Just input needs and let AI do the rest.                   
 **Smarter Contracts**       | Analyze smart contracts and tokens on Ethereum.                                                                              
 **SmartTicketsAI**          | Get Tickets For All Sports Events, Music Concerts, Theater And More With SmartTicketsAI.com.                                 
 **Snappy Gifts**            | Send curated gift collections and let your recipient choose their favorite gift.                                             
 **Social Search**           | The Social Search provides access to tweets, users, followers, images, media and more.                                       
 **Solana Labs**             | Explore Solana blockchain data.                                                                                              
 **SourceScoutAI**           | Identify sources and experts for journalists.                                                                                
 **Space**                   | Space data including NASA.                                                                                                   
 **Space Photo Explorer**    | Tool for exploring space through images including Mars Rover Photos, NASA image database, and space pictures of the day.     
 **Space Server**            | Access NASA’s technology transfer, Mars weather, SSD/CNEOS, Nasa Photos – By Nabil Brag.                                     
 **SPARQL Query**            | Returns Query Solution Page for Query Text.                                                                                  
 **Speak**                   | Learn how to say anything in another language with Speak, your AI-powered language tutor.                                    
 **Speechki**                | The easiest way to convert texts to ready-to-use audio — download link, audio player page, or embed!                         
 **Spendless**               | Discover personalized Amazon products at unbeatable prices. Now available in the US.                                         
 **Spirify QR Code**         | QR Code generator for text or URLs. It’s a general-purpose tool that also enables QR code puzzle games with AI.              
 **Splitgraph**              | Search thousands of public datasets available on the Splitgraph Data Delivery Network.                                       
 **Sponsored.pro**           | Product photography prompt generator. Start your prompt with ‘Sponsored’ followed by a few words about colors,…              
 **SSENSE**                  | Enjoy personalized product recommendations from over 500 luxury and lifestyle brands.                                        
 **StackOverflow Plus**      | Expand your knowledge beyond 2021/09/01. Get recent questions and answers from Stack Overflow.                               
 **Statis Fund Finance**     | Financial data tool for analyzing equities. You can get price quotes, analyze moving averages, RSI, and more.                
 **Staypia**                 | Find your perfect travel destination &amp; hotels. Look for pet-friendly towns in USA, stargazing spots in Europe, and more! 
 **StealthTest**             | Ship web3 apps &amp; NFTs faster with StealthTest: Ethereum, Polygon, &amp; Moonbeam private testnet.                        
 **SteamReviewsWatcher**     | Get steam reviews for the requested game and process them as you want, make summary’s, extract pros and cons, etc.           
 **Stepstone Jobs**          | Explore job openings in Germany. 120,000+ opportunities to discover.                                                         
 **Stoic**                   | Get a stoic quote by typing ‘stoic’.                                                                                         
 **Stories**                 | Create beautiful, illustrated stories easily.                                                                                
 **Storybooks**              | Create bedtime stories that instill a love for reading, spark creativity, and build important life skills.                   
 **Strology**                | Get daily astrological predictions for your sun sign.                                                                        
 **Substack IQ**             | Explore Substack! Provide a link, chat with articles, find new authors, search all of Substack, &amp; more.                  
 **Sudoku**                  | This is a sudoku game. You use voice or text to play.                                                                        
 **Supercharger**            | Awesome Tesla trips: find a route, including supercharger stops. Bonus: chat with the Tesla manual!                          
 **SuperColleague**          | Let AI tools do your work.                                                                                                   
 **Supermetrics**            | Effortlessly tap into Facebook Ads data &amp; gain insights. Trusted by 1M+ marketers worldwide.                             
 **SuperStock Fine Art**     | Returns a set of URLs to examples of fine art requested by user.                                                             
 **Surveillance API**        | Explore the Synology Surveillance Station API documents and generate example code.                                           
 **SurveySparrow**           | Create and deploy interactive surveys effortlessly with AI for valuable insights.                                            
 **Swft cross chain**        | Querying cross chain info and swap coins.                                                                                    
 **Tabechoku**               | We propose the perfect peaches for you from all over Japan.                                                                  
 **Tabelog**                 | Allows you to find restaurants in Japan that have availability for reservations.                                             
 **Table Labeler**           | Perform labeling, data extraction and text transformation, for Excel or CSV spreadsheet columns.                             
 **Tabor – Medicare**        | Provides info on Medicare plans, eligibility, and benefits. Helps users make informed decisions.                             
 **Tabor – Sr. Living**      | Trusted source for senior living market research, data, and analytics. 35K communities, 9K operators in the USA.             
 **Tagvenue**                | Search for event venues and spaces in Australia, Canada, Ireland, Singapore, the UK, and the USA.                            
 **Tailor ERP Generator**    | A tool to help creating tailor-made ERP application.                                                                         
 **Take Code Captures**      | Capture, beautify, and share your code snippets easily.                                                                      
 **TalentOrg**               | Find and hire freelance engineering talents from around the world.                                                           
 **Talk Law Brazil**         | Interact with laws and bills in progress in the Brazilian Congress.                                                          
 **Talk With Code**          | Send GitHub link and ask any aspect of the code.                                                                             
 **Talk With Docs**          | Ask any questions to your docs.                                                                                              
 **TalkFPL**                 | Talk with AI to help you manage your FPL team. Compare players, transfer options and more.                                   
 **Tan Pilot**               | Provides optimized web access ability.                                                                                       
 **Tarot**                   | Experience powerful, personalized readings. Let the cards give you spiritual insight and guidance.                           
 **Taskade**                 | Taskade, the AI-Powered Productivity Platform.                                                                               
 **TaskML**                  | Manage your projects and tasks. Quickly create projects, add tasks / todos, and mark them as done.                           
 **TaskOracle**              | Hey Neo, take the red pill less, let me guide you with the optimal tools to smash your to-do list like Hulk.                 
 **Tasty Recipes**           | Discover recipe ideas, meal plans and cooking tips from Tasty’s millions of users!                                           
 **TechPulse**               | Keeps you updated with top(default)/new/ask/show/job stories from HackerNews.                                                
 **Telegraph Export**        | Export text to Telegraph.                                                                                                    
 **Telescope Labs**          | Data retrieval, analysis and visualizations related to web3 games who are onboarded as a blockchain game.                    
 **Testosterone Meals**      | Designed diet plans that aim to boost your testosterone levels naturally.                                                    
 **Text Count**              | Count the number of words and characters in a text.                                                                          
 **The Bolt**                | Energize your chat with tools from thebolt.ai that enhance your productivity and workflow.                                   
 **The Diet Search**         | You can search the Japanese Diet Proceedings by request, news, or other text.                                                
 **The Knot**                | All-In-One wedding planning with tools, guides, &amp; inspo to make your wedding journey effortless.                         
 **The Remembotron**         | Simple long term memory. Store lists, tasks, and other information. Recover it in other chats when you need it.              
 **ThemeParkHipster**        | Find theme park waiting times around the world.                                                                              
 **There’s An AI For It**    | Find the right AI tools for any use case, from the world’s largest database of AI tools.                                     
 **ThinkSchool by Spex**     | Chat with the Think School library on Spext.                                                                                 
 **TianGong**                | Search through academic papers and professional documents.                                                                   
 **Tic Tac Toe**             | Playing a game of Tic Tac Toe with varying board sizes. You can submit your move and get the AI’s response move.             
 **TickTick**                | TickTick for managing a TODO list, you can add, remove and view your TODOs.                                                  
 **TILNOTE**                 | Search your notes and answer your question. Write a note and embed it for retrieval using tilnote.io.                        
 **TimeMachine**             | Enhances AI with real-time awareness, providing current time in various formats and timezones.                               
 **TimeNavi**                | Interact with your calendar. Analyze and create events, understand and plan your time better.                                
 **Timeport**                | Begin an exciting journey through time, interact with unique characters, and learn history in this time-travel game!         
 **Tint Bolt**               | Tint Bolt is an administrative tool that helps automotive window tinting businesses manage their day to day operations.      
 **Tira**                    | Shop Tira for top beauty brands! Explore cosmetics, health products &amp; more online. Your beauty store awaits              
 **TL;DR**                   | TL;DR                                                                                                                        
 **Today’s Top Deal**        | Find the best deals on Amazon today.                                                                                         
 **TokenInsights**           | Get realtime crypto price, BTC, ETH, BNB, and the latest insights.The latest coin news and airdrop opportunities.            
 **Tokyo OpenData**          | Search for OpenData in Tokyo, using the Tokyo OpenData Catalog Site.                                                         
 **Tomorrow.io Weather**     | Predicting, planning, and adapting to the weather forecast via contextualized chat-based insights.                           
 **TomTom**                  | Explore maps, get traffic updates, find destinations, plan routes and access local insights.                                 
 **Too Long Didn’t Read**    | Enter any URL and have the text content summarized for you!                                                                  
 **Toolblox**                | Text to smart-contract. Describe a process to generate a smart-contract and deploy to any blockchain.                        
 **Top Agencies**            | Find top marketing and design agencies around the World by service, locations, and ratings.                                  
 **Topical Authority**       | AISEO topical authority tool will generate the topical authority for any topic and helps you dominate your                   
 **TotalQuery Search**       | Go beyond google search: harness the combined power of 70+ search engines for ultimate web discovery.                        
 **TR Timetables**           | Query for the ID for Taiwan Railway stations and timetables for the specified departure and destination station.             
 **Traders Insight**         | Decode the latest technical analysis ideas for stocks and bitcoin from top traders.                                          
 **TradingBro**              | Get financial data for your trading/learning: earning call, analyst view, DCF, sales details, insider trading etc.           
 **Trambellir.com**          | Online Marketplace for Wellness holidays.                                                                                    
 **TranslationRater**        | How good is a translation? Prompt for translation or type your own. Welocalize TranslationRater will evaluate it.            
 **Travel Planning**         | Travel Planning assists in planning your trip. It generates a map with all the places and webpages for you to share.         
 **TRAVELKO**                | From worldwide sites, search &amp; compare the lowest prices for Japanese hotels matching your wishes.                       
 **Travelmyth**              | Unleash personalized hotel search with Travelmyth, offering 60 unique categories for the perfect match.                      
 **Trax Codes**              | Access the largest collection of up-to-date Canadian building codes and other regulations for the construction industry.     
 **Trending Music**          | Service for finding the trending music, by genre, countries, and top songs globally.                                         
 **Trip.com**                | Discover the ultimate travel companion-simplify your trip with personalized itinerary planning and travel bookings.          
 **Tripadvisor**             | Discover the finest hotels, restaurants, attractions, and experiences for your next adventure!                               
 **Tryp.com**                | Tryp.com’s AI-driven inspirational travel engine, allows adventurous souls to travel way more, for way less!                 
 **TrySpree**                | TrySpree is a website that finds free samples for you to try.                                                                
 **TTS 11labs**              | Convert text to speech with diverse voices &amp; models. Easy to use for narration, games &amp; more.                        
 **Tubi**                    | Watch Free Movies and TV Shows Online.                                                                                       
 **Turing Developer**        | Search and hire the world’s most deeply vetted developers from Turing.com.                                                   
 **Turo**                    | Search for the perfect Turo vehicle for your next trip.                                                                      
 **Tutory**                  | Access affordable, on-demand tutoring and education right at your fingertips.                                                
 **twtData**                 | Get Twitter data on any public Twitter account’s including their name, bio, follower count and much more.                    
 **txyz.ai**                 | Effortlessly decipher, compare, and answer questions about research papers using a simple Arxiv ID.                          
 **Typeform**                | Create forms, quizzes, and surveys—designed to stand out.                                                                    
 **TypeScript Analyzer**     | Analyzes TypeScript, JSX/TSX, and JavaScript files for linting, autocompletion, error checking, and                          
 **Uberchord**               | Find guitar chord diagrams by specifying the chord name.                                                                     
 **UK Latest News**          | Get the latest news stories from the UK’s top news outlets including BBC News, Sky News, The Independent, and                
 **UK Politics**             | Search through UK political documents such as speeches, press releases, voting records, and candidates’ profiles.            
 **Ukr-School-Books**        | Interact with a books database. List, get content, and find books.                                                           
 **Universal**               | Enables to access web pages, analyze PDFs, generate QR codes, etc.                                                           
 **UOL**                     | I deliver a selection of the main news and the important events of the day to inform you. Native language:…                  
 **Upskillr**                | Custom curriculum, lesson plans, and upskilling support on any topic by Upskillr. Powered by Shorthills Tech.                
 **Urban Ladder**            | Search &amp; Browse from Urban Ladder Product Catalog.                                                                       
 **URL Data Scraper**        | Rapidly get text, PDF, or images from any url.                                                                               
 **US Federal Law**          | Get up to date access to Federal law.                                                                                        
 **UwU Translator**          | Use this to translate text into ‘UwU’ language.                                                                              
 **VainSoftGames**           | Plugin for all your video game needs. Details on the latest games, including images, reviews &amp; videos.                   
 **ValidatorAI**             | Get objective feedback and an elevator pitch for your business idea.                                                         
 **VBOUT AI MARKET**         | Create email campaigns, manage leads and schedule social posts in VBOUT, using simple prompts.                               
 **Vehicle Data IL**         | An Israel-focused tool, extracting car details from data.gov.il based on model, year, hue, and count.                        
 **VELTRA**                  | A tool designed to provide personalized tour activity suggestions.                                                           
 **Veridas Docs**            | Answers questions about Veridas products using the official documentation.                                                   
 **Video AI by invideo**     | Transform text to stunning videos with invideo AI. Your video copilot for YouTube, TikTok, Instagram Reels, and…             
 **Video Captions**          | Convert Youtube links into transcribed text, enable asking questions, create chapters, and summarize its content.            
 **Video Highlight**         | Explore, research, and interact with YouTube videos and personal videos.                                                     
 **Video SEO Copilot**       | I’ll write &amp; update YouTube video descriptions from transcripts, identify SEO keywords, and boost video                  
 **Video Summary**           | I’ll create YouTube video summaries from transcripts and provide concise content overview with timestamps.                   
 **Video Summary**           | Summarize YouTube video highlights. Generate summaries from YouTube video URLs.                                              
 **VideoInsights.io**        | VideoInsights.io lets you interact with online video platforms like Youtube, Zoom or BiliBili.                               
 **vidIQ**                   | Discover and summarize trending videos on YouTube.                                                                           
 **Vio.com**                 | A better deal on your next hotel, motel or accommodation booking.                                                            
 **Visla**                   | Turn your idea into a captivating video. AI scripts, selects stock footage, and creates magic!                               
 **Visualize Your Data**     | Instantly transform your raw data into stunning, insightful visuals and charts.                                              
 **Visuogen**                | Visualize your CSV data with charts. Create, edit, and evaluate effortlessly.                                                
 **Vivian Health**           | Take the first step to finding your next healthcare job.                                                                     
 **Vivid Seats**             | Discover local events and find tickets with Vivid Seats.                                                                     
 **VoiceOver**               | Convert text into natural-sounding speech.                                                                                   
 **VoiExplorer**             | Voi network explorer and search with enriched augmented data. Built by GoPlausible!                                          
 **VOMO**                    | Type ‘record’ or ‘create’ to take notes and save them to VOMO. Type ‘save to vomo’ to save the current conversation.         
 **VoxScript**               | Enables searching of YouTube transcripts, web browsing/searching, and more!                                                  
 **VPLATE Video Ads**        | Hey AI, Create Video Ads – Enter Product Info, Get Stunning Video Ads in a Snap!                                             
 **Wahi**                    | Hey Ontario, ask and get so in the know on the latest listings, property insights and more.                                  
 **Wallet Passes**           | Create Wallet Passes for iOS device (iPhone, iPod, iWatch) with ease!                                                        
 **Wanted Job Search**       | Explore and inquire about global job opportunities, and dive into the details of worldwide positions with precision.         
 **Wantedly Visit**          | It will help you find a job in Japan or Singapore.                                                                           
 **Water Tracker**           | A tool for recording and tracking your water intake records, drink more water, stay healthy.                                 
 **Weather**                 | Get the weather forecast for a US city or zip code. Visit https://weatherplugin.net for more details.                        
 **Weather**                 | Provides weather forecast based on location. Includes temperature, precipitation, cloud cover, wind and much more.           
 **Weather Meteosource**     | Accurate weather for chat-based insights – current, forecasted, and historical. Updated in real-time.                        
 **Weather Report**          | Current weather data for cities and airports using METAR aviation feeds.                                                     
 **WeatherWhiz**             | WeatherWhiz: Accurate current weather and forecasts for any location.                                                        
 **WeatherWizard**           | Get current weather information from around the world. Powered by WeatherAPI.                                                
 **Web Reader**              | Read and search data from URL link to website / webpage. Supports PDF, Word files, Excel, CSV, Google Drive and more.        
 **Web Requests**            | Goodbye Knowledge Cutoff, Hello World! This is your AI assistant’s web browser. Just enter a URL. Google, Wiki, GitHub.      
 **Web Search AI**           | Real time search results. Browse and summarize multiple pages with AI scraping.                                              
 **Web3 User Activity**      | View any Web3 user’s activities by entering a 0x address, ENS, Lens, and many other name services on the RSS3 Network.       
 **Web5 assistant**          | Assistant for developers building a web5 application by providing examples, documentation and writing web5 code.             
 **Webbots – Ask a Bot**     | Start with ‘Ask bot &lt;bot id&gt;’. Visit webbot.chat to create &amp; share a free bot. Powered by Maigrate.                
 **WebDev**                  | Build a live website within seconds directly from the chat and preview and test HTML code with JavaScript and CSS.           
 **Webpage Summarizer**      | Allow the bot to access information contained in a webpage via its URL, by default returning a summary.                      
 **WebPilot**                | Browse Webpage/PDF/Data. Chat &amp; Write with one/many/none URLs. WebPilot.ai Powered.                                      
 **WebRewind**               | Get the picture of a website at a specific date.                                                                             
 **Website Builder**         | Generate websites for businesses in Chat, presentations, finance, charts, diagrams, document,pdf.                            
 **Website Performance**     | Measure key metrics about your website – performance, accessibility, best practices, SEO, PWA.                               
 **websitestoreAI**          | An interactive plugin for websitestore.                                                                                      
 **Welt NewsVerse**          | Stay informed with the latest news from welt.de.                                                                             
 **What To Watch**           | Search for current TV shows and movies, get recommendations, and find out where things are streaming.                        
 **What’s trending?**        | Explore trending searches, keywords, &amp; hashtags by topic across YouTube, Instagram, Amazon, and Google.                  
 **Whois Domain Checker**    | A Whois app that takes a list of space-separated domain names, performs the Whois for each domain.                           
 **WikiDocs**                | You can search for books on Wikidocs and create books.                                                                       
 **Wikipedia**               | Ask questions about general knowledge, current events, and breaking news, and get up-to-date answers from Wikipedia.         
 **Wisecube Orpheus**        | Wisecube AI plugin lets users search the largest biomedical knowledge base and access billions of facts.                     
 **Wishbucket**              | Unified product search across all Korean platforms and brands.                                                               
 **WizeCharts**              | Create and share beatiful and highly customizable charts.                                                                    
 **Wolfram**                 | Access computation, math, curated knowledge &amp; real-time data through Wolfram|Alpha and Wolfram Language.                 
 **Word Counter**            | Count the number of words, and characters (with and without spaces).                                                         
 **Word Sneak**              | The AI has to sneak 3 secret words into your conversation. Guess the words to win the game!                                  
 **WordCloud**               | Create word cloud images from text.                                                                                          
 **WORDLY – WORD Game**      | Play Guess the WORD AI game. You need to guess a 5 letter word! Start by asking to play WORDLY game.                         
 **Words Pro**               | Dive into an endless sea of five-letter mysteries. Can you decode them all?                                                  
 **Workout Generator**       | Create strength training workouts, for goals like muscle building, strength, and fat loss.                                   
 **Workoutlytics**           | Your personal guide to peak performance with Strava. Track heart rate, progress &amp; tips.                                  
 **World Bank Data**         | Access global data on development, economics, demographics, and more from the World Bank Datasets using a query term.        
 **World News**              | Summarize news headlines. You can ask for the latest news from various sources around the world.                             
 **World Records**           | Step into a world that celebrates the highest levels of human accomplishment. Witness the records!                           
 **WOXO**                    | Turn your prompts into cool videos. Create eye-catching content in a snap!                                                   
 **WP Interact**             | Fetch or search posts from self-hosted WordPress websites, opening new possibilities for smart interaction with              
 **WPressGenie**             | Manages a WordPress site. You can get, view and analyze details of WordPress posts and content.                              
 **WriteEasy**               | Get AI generated articles by providing a topic and links to use for inspiration or data.                                     
 **xkcd AI**                 | Immerse yourself in the humor and wisdom of xkcd comics with personalized explanations.                                      
 **Xpapers**                 | Effortlessly find real academic papers on arXiv. Dive into abstracts, references, and access public PDF URLs.                
 **XtalPi**                  | Provide drug discovery solutions and chemical services from XtalPi, your drug discovery partner.                             
 **Xweather**                | XWeather gives weather information for a location. Ask for the current weather, a 5-day forecast, or a radar image.          
 **Yabble**                  | Create insights instantly. Any audience. Any question. Yabble it.                                                            
 **Yay! Forms**              | Allows you to create AI-Powered Forms, Surveys, Quizzes, or Questionnaires on Yay! Forms.                                    
 **Your AI Council**         | The AI council assesses queries through various agents, offering insights from many perspectives.                            
 **YouTube Summaries**       | Insights &amp; Summaries for YouTube videos.                                                                                 
 **YT Caption Retriever**    | Tool for retrieving YouTube video captions. You can retrieve captions from any YouTube video by providing its URL.           
 **YT Summarizer**           | 💯Free YouTube Summarizer: Saves a lot of screen time by summarizing YouTube videos with timestamps.                         
 **Z3 Checker**              | Calls the Z3 verifier on an SMT-LIB specification.                                                                           
 **Zapier**                  | Interact with over 5,000+ apps like Google Sheets, Gmail, HubSpot, Salesforce, and thousands more.                           
 **Zefira Thoth Tarot**      | Draw and interpret Tarot cards, and perform text-based search on Tarot card information.                                     
 **Zeo Route Planner**       | Plan your routes efficiently with Zeo Route Planner. Perfect for deliveries, road trips, or city tours.                      
 **ZeroBounce**              | Validate email addresses using the ZeroBounce API.                                                                           
 **Zhibo8 Sports**           | Provides schedules for NBA, CBA, CSL, Premier League, Serie A, La Liga, Bundesliga, and Ligue 1.                                                                          

## Third-party Plugins

- [humanjobs](https://github.com/woniesong92/humanjobs) - A ChatGPT Plugin that lets ChatGPT create job postings only for humans.
- [ethgpt](https://github.com/xinbenlv/ethgpt) - ChatGPT Plugin for Ethereum and EVM dApp / smart contract functionalities.
- [gpt-terminal-plugin](https://github.com/etherlegend/gpt-terminal-plugin) - ChatGPT Universal Terminal / Command Line Plugin.
- [bra1nDump](https://github.com/bra1nDump/show-me-chatgpt-plugin) - Create and edit diagrams in seconds without even leaving the chat.
- [chatgpt-code-plugin](https://github.com/kesor/chatgpt-code-plugin) - A TypeScript Code Analyzer that enables ChatGPT to "talk" with YOUR code.
- [kaguya](https://github.com/ykdojo/kaguya) - A ChatGPT plugin that allows you to load and edit your local files in a controlled way, as well as run any Python, JavaScript, and bash script.

## More Resources

- [ChatGPT-Resources](https://github.com/jqueryscript/ChatGPT-Resources) - Curated list of awesome libraries, extensions, tools, apps for ChatGPT.
- [alt-gpt](https://github.com/feedox/alt-gpt) - Playground for developing ChatGPT plugins.
